...
6. **Drizzle ORM only.** Schema changes go in `packages/db/src/schema/`. Run `pnpm db:generate`. Never write raw SQL migration files. (Decision #19) **Narrow exception (C14) — TWO classes, and nothing else.** Both are things drizzle-kit provably cannot emit under this repo's config.
   ...
   **(b) Cluster/security DDL** — roles, `GRANT`/`REVOKE`, `ENABLE`/`FORCE ROW LEVEL SECURITY`, `CREATE POLICY`, and functions plus their ACLs — hand-authored into an EMPTY `--custom` stub (`pnpm db:generate --custom --name=…`), because there is no schema delta to diff onto. `drizzle.config.ts` declares no `entities.roles` and no `pgPolicy`, and drizzle-orm's `pg-core` exposes no function/routine primitive, so no generator route exists for any of it. 26 migrations do this today — 0211 (the tenant-RLS foundation), 0213/0214 (serving-role hardening), 0259/0261 (app-select grants), 0266 (the canary preflight's `SECURITY DEFINER` evidence function). Every such migration must state in a header comment WHY drizzle-kit cannot emit it, and every statement must be idempotent (`CREATE OR REPLACE`, `DO $$ IF NOT EXISTS`, `DROP POLICY IF EXISTS` before `CREATE POLICY`; `GRANT`/`REVOKE` are natural no-ops on re-apply).

   **Table, column, index and constraint DDL is NEVER hand-authored** — that is always `db:generate` output, diffed from `packages/db/src/schema/`. A `--custom` migration that touches it is the violation this rule exists to stop.

   *(History: this clause previously described only class (a) and then said "schema DDL is never hand-authored", which read as a blanket ban. Class (b) had been established practice for 26 migrations — the whole E2 tenant kernel depends on it — but was never written down, so a Codex review of PR #333 correctly flagged a compliant migration as a violation. The prohibition is unchanged; it is now stated in terms of the object classes it actually covers.)*
