...
**Read visibility (RBAC, Decision #118):** retrieval is gated per actor — `memory-access.ts` `canActorSee` / `memory-access-sql.ts` `memoryAccessConditions` (ORG/CREW/MCP paths), `memory-policy.ts` `canSeeDurableMemory` (Commander). **identity** → agents (grounding) + founder/team_lead; NOT team_member humans, NOT external MCP keys. **visibility='company'** → every internal member (incl. team_member); NOT external MCP keys. **domain/active_context/working** → scope-matched (department/project/goal→project/task→project). **Private** (owner-typed or agent-personal) → owner-only; founder never sees others' private (break-glass is separate). **Invalidated** → nobody; **founder** sees all non-private. External MCP keys resolve to a scoped `team_member` marked `external:true` and are excluded from identity+company. Do NOT re-broaden without a successor decision.
