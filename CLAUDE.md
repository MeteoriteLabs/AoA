...
- **Company portability:** [IMPLEMENTED — Phase E.1] Export/import full company bundles at Paperclip parity. Bundle `schemaVersion: 2` covers company, agents, projects (departments + projects), issues/tasks, skills (with file inventory + source tracking), routines (triggers + variables), envInputs (secret/plain/portability classification). `POST /:companyId/export/preview` returns counts + file list + size estimate before building. README.md generated per export (`company-export-readme.ts`). Unknown bundle sections warn-and-continue (no silent data loss) — v1 Paperclip bundles import compatibly. UI: `/export` (entity selection → preview → JSON download) + `/import` (upload → preview plan → collision strategy → import). AoA-only entities (memory, artifacts, discussions, budgets) land in Phase E.2.
