...
- **MCP bidirectional:** V1 inbound routed to Discussion. V2 exposes full AoA-as-MCP-server with read resources (tasks, goals, memory, artifacts, agents, projects, comments, approvals) + write tools (create-task, update-task, add-task-comment, attach-artifact-version, suggest-memory, debrief-push, approval decisions). RBAC-scoped per user. (Decision #14 revised)
