...
1. **Drizzle ORM only.** Schema changes go in `packages/db/src/schema/`. Run `pnpm db:generate` for migrations. NEVER write raw SQL migration files. Schema DDL is always `db:generate` output. **Narrow exception (C14) — TWO classes, both of which drizzle-kit provably cannot emit:** **(a) idempotency guards** (`IF NOT EXISTS` / `DO $$ … duplicate_object`) **and data-only backfills**, hand-APPENDED below generated DDL (e.g. `0189`, `0195`); **(b) cluster/security DDL** — roles, `GRANT`/`REVOKE`, `ENABLE`/`FORCE ROW LEVEL SECURITY`, `CREATE POLICY`, and functions plus their ACLs — hand-authored into an empty `--custom` stub with no schema delta to diff onto (e.g. `0211`, `0213`, `0214`, `0261`, `0266`; 26 migrations and counting). This repo's `drizzle.config.ts` declares no `entities.roles` and no `pgPolicy`, and drizzle-orm's `pg-core` exposes no function primitive, so there is no generator route for class (b) at all. Both classes require an inline comment and must be idempotent. **Table, column, index and constraint DDL is never hand-authored** — that is always `db:generate` output. See `AGENTS.md` and Decision #19.
