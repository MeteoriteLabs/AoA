...
export function PluginPage() {
  ...
  const { data: contributions } = useQuery({
    queryKey: queryKeys.plugins.uiContributions(resolvedCompanyId ?? ""),
  ...
  useEffect(() => {
    if (pageSlot) {
      setBreadcrumbs([
        { label: "Plugins", href: "/instance/settings?tab=plugins" },
  ...
  if (!pageSlot) {
    ...
    const settingsPath = pluginId ? `/instance/settings/plugins/${pluginId}` : "/instance/settings?tab=plugins";
