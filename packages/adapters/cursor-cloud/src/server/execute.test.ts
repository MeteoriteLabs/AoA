...
describe("cursor_cloud execute", () => {
  ...
  it("rejects AoA remote execution targets because Cursor Cloud owns its runtime target", async () => {
    const ctx = createContext({
      executionTarget: {
    ...
    expect(result.errorMessage).toContain("cannot execute inside provider-sandbox");
  });
});
