...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (adapterExecutionTargetIsRemote(executionTarget)) {
    return {
      ...
      timedOut: false,
      errorMessage:
        `cursor_cloud runs through Cursor Cloud and cannot execute inside ${describeAdapterExecutionTarget(executionTarget)}. ` +
        "Use the adapter's Cursor runtime environment fields instead, or choose a local CLI adapter for AoA sandbox execution.",
      ...
      resultJson: {
        executionTarget,
        supportedTargets: ["cursor_cloud"],
      },
      summary: "cursor_cloud does not support AoA remote execution targets.",
      clearSession: false,
    };
  }
