...
export async function testEnvironment(
  ...
  if (adapterExecutionTargetIsRemote(executionTarget)) {
    checks.push({
      ...
      level: "error",
      message: `acpx_local cannot execute inside ${describeAdapterExecutionTarget(executionTarget)}.`,
      detail: "ACPX is embedded in the AoA server process; it does not use the adapter-utils remote process runner.",
      hint: "Use ACPX with a local/hosted AoA runtime, or choose claude_local/codex_local for sandbox-backed execution.",
    });
  }
