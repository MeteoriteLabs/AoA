...
describe("acpx_local runtime skill isolation", () => {
  ...
  it("rejects remote execution targets because ACPX runs embedded in the AoA server process", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    ...
    const execute = createAcpxLocalExecutor({
      createRuntime: () => {
        throw new Error("remote ACPX should not start the embedded runtime");
      },
    });

    const result = await execute({
      runId: "run-remote-acpx",
      agent: {
        id: "agent-1",
        companyId: "company-1",
      },
    ...
    expect(result.errorCode).toBe("unsupported_execution_target");
    expect(result.errorMessage).toContain("cannot execute inside sandbox-docker");
    expect(result.resultJson).toMatchObject({
      supportedTargets: ["local"],
    });
    expect(await pathExists(path.join(stateDir, "wrappers"))).toBe(false);
  });
  ...
  it.skipIf(process.platform === "win32")("opts the claude agent into ACPX runtime verbose logs but leaves codex/custom agents quiet", async () => {
    const root = await makeTempRoot();
    const cwd = path.join(root, "worktree");
    await fs.mkdir(cwd, { recursive: true });
  ...
  it.skipIf(process.platform === "win32")("does not touch .claude/settings.local.json for the codex agent", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    const cwd = path.join(root, "worktree");
    await fs.mkdir(cwd, { recursive: true });
