  ...
  return async function executeAcpxLocal(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    if (adapterExecutionTargetIsRemote(executionTarget)) {
      return {
        ...
        timedOut: false,
        errorMessage:
          `acpx_local runs the embedded ACPX runtime in the AoA server process and cannot execute inside ${describeAdapterExecutionTarget(executionTarget)} yet. ` +
          "Use a local/hosted AoA runtime for ACPX, or choose claude_local/codex_local for sandbox execution.",
        ...
        summary: "acpx_local does not support remote execution targets yet.",
      };
    }
