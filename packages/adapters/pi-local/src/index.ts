import type { AdapterModelProfileDefinition } from "@armyofagents/adapter-utils";
...
Use when:
- You want AoA to run Pi (the AI coding agent) locally as the agent runtime
...
- AoA requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/aoa/ and resumed with --session.
