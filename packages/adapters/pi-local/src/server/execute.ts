...
import { inferOpenAiCompatibleBiller, type AdapterExecutionContext, type AdapterExecutionResult } from "@armyofagents/adapter-utils";
import {
...
const AOA_SESSIONS_DIR = path.join(os.homedir(), ".pi", "aoa");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[aoa] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[aoa] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[aoa] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(AOA_SESSIONS_DIR, { recursive: true });
  return AOA_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(AOA_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // invoke skill binaries (e.g. `aoa-get-issue`) by name. Without this,
  ...
  await ensureAdapterExecutionTargetRuntimeCommandInstalled({
    ...
    runShellCommand: (command) =>
      runAdapterExecutionTargetShellCommand(runId, executionTarget, command, {
        cwd,
        env: runtimeEnv,
        timeoutSec,
        graceSec,
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[aoa] Syncing workspace and Pi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
      ...
      if (localSkillsDir && remoteRuntimeRootDir) {
        remoteSkillsDir = `${remoteRuntimeRootDir}/skills`;
        await syncAdapterExecutionTargetDirectory({
          runId,
          target: executionTarget,
          localDir: localSkillsDir,
          remoteDir: remoteSkillsDir,
  ...
  if (runtimeSessionId && !canResumeSession) {
    const staleSessionCwdNote =
      savedSessionCwd !== null && !sessionHeaderCwdMatches
        ? ` Pi stored cwd "${savedSessionCwd}" in the session header, so AoA will start a fresh session for "${effectiveExecutionCwd}".`
        : "";
    await onLog(
      "stdout",
      executionTargetIsRemote
        ? `[aoa] Pi session "${runtimeSessionId}" does not match the current remote execution state and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote} Starting a fresh remote session.\n`
        : `[aoa] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote}\n`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[aoa] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
      );
