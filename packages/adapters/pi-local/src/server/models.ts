...
import { asString, runChildProcess } from "@armyofagents/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AOA_PI_COMMAND === "string" &&
    process.env.AOA_PI_COMMAND.trim().length > 0
      ? process.env.AOA_PI_COMMAND.trim()
...
const VOLATILE_ENV_KEY_PREFIXES = ["AOA_", "PAPERCLIP_", "npm_", "NPM_"] as const;
