...
vi.mock("@armyofagents/adapter-utils/server-utils", async () => {
  const actual = await vi.importActual<typeof import("@armyofagents/adapter-utils/server-utils")>(
    "@armyofagents/adapter-utils/server-utils",
  );
  return {
    ...actual,
    ensureCommandResolvable,
...
vi.mock("@armyofagents/adapter-utils/ssh", async () => {
  const actual = await vi.importActual<typeof import("@armyofagents/adapter-utils/ssh")>(
    "@armyofagents/adapter-utils/ssh",
  );
  return {
    ...actual,
    prepareWorkspaceForSshExecution,
...
vi.mock("@armyofagents/adapter-utils/execution-target", async () => {
  const actual = await vi.importActual<typeof import("@armyofagents/adapter-utils/execution-target")>(
    "@armyofagents/adapter-utils/execution-target",
  );
  return {
    ...actual,
    startAdapterExecutionTargetPaperclipBridge,
