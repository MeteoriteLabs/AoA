...
Use when:
- You want AoA to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the AoA server.
...
- paperclipApiUrl (string, optional): absolute AoA base URL advertised in wake text
...
- paperclip (object): standardized AoA compatibility context added to every gateway agent request
- paperclip.workspace (object, optional): resolved execution workspace for this run
- paperclip.workspaces (array, optional): additional workspace hints AoA exposed to the run
