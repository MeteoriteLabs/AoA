...
export async function testEnvironment(
  ...
  if (adapterExecutionTargetIsRemote(executionTarget)) {
    checks.push({
      ...
      level: "error",
      message: `openclaw_gateway cannot execute inside ${describeAdapterExecutionTarget(executionTarget)}.`,
      detail: "The adapter talks to OpenClaw Gateway over WebSocket from the AoA server process.",
      hint: "Configure the gateway URL/auth for the OpenClaw runtime, or choose a local CLI adapter for AoA sandbox execution.",
    });
    return {
