...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (adapterExecutionTargetIsRemote(executionTarget)) {
    return {
      ...
      timedOut: false,
      errorMessage:
        `openclaw_gateway talks to an OpenClaw Gateway over WebSocket and cannot execute inside ${describeAdapterExecutionTarget(executionTarget)}. ` +
        "Configure the gateway URL/auth for the target OpenClaw runtime, or choose a local CLI adapter for AoA sandbox execution.",
      ...
      summary: "openclaw_gateway does not support AoA remote execution targets.",
    };
  }
