import { asNumber, asString, parseJson, parseObject } from "@armyofagents/adapter-utils/server-utils";
...
export function parseGeminiJsonl(stdout: string) {
  ...
  for (const rawLine of stdout.split(/\r?\n/)) {
    ...
    if (type === "result") {
      ...
      const eventStatus = asString(event.status, "").toLowerCase();
      const isError = event.is_error === true ||
        asString(event.subtype, "").toLowerCase() === "error" ||
        eventStatus === "error" || eventStatus === "failed";
      if (isError) {
