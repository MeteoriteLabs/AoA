...
export const secretAccessEvents = pgTable(
  ...
  (table) => ({
    ...
    companyConsumerIdx: index("secret_access_events_company_consumer_idx").on(
      table.companyId,
      table.consumerType,
      table.consumerId,
    ),
    heartbeatRunIdx: index("secret_access_events_heartbeat_run_idx").on(table.heartbeatRunId),
  }),
);
