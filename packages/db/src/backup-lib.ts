...
import type { BackupRetentionPolicy, BackupRetentionTieredPolicy } from "@armyofagents/shared";
...
const STATEMENT_BREAKPOINT = "-- aoa statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900";
...
export function pruneOldBackups(
  ...
  // Count mode: keep at most N backups, newest first
  if ("mode" in policy && policy.mode === "count") {
    const maxCount = Math.max(1, Math.trunc(policy.count));
  ...
  // Tiered mode
  const tieredPolicy = policy as BackupRetentionTieredPolicy;

  // Daily tier: newest backup per day for last N days
  const dayCutoff = new Date(now);
  dayCutoff.setUTCDate(dayCutoff.getUTCDate() - tieredPolicy.dailyDays);
  ...
  weekCutoff.setUTCDate(weekCutoff.getUTCDate() - tieredPolicy.weeklyWeeks * 7);
  ...
  monthCutoff.setUTCMonth(monthCutoff.getUTCMonth() - tieredPolicy.monthlyMonths);
  const seenMonths = new Set<string>();
  for (const b of sorted) {
    if (b.createdAt >= weekCutoff || b.createdAt < monthCutoff) continue;
    const monthKeyVal = calendarMonth(b.createdAt);
    if (!seenMonths.has(monthKeyVal)) {
      seenMonths.add(monthKeyVal);
      keep.add(b.path);
    }
  }
...
function pruneBackupFiles(backupDir: string, policy: BackupRetentionPolicy, filenamePrefix: string): number {
  ...
  if ("mode" in policy && policy.mode === "count") {
    ...
    for (const e of toDelete) unlinkSync(e.fullPath);
  ...
  const tieredPolicy = policy as BackupRetentionTieredPolicy;
  const now = Date.now();
  const dailyCutoff = now - Math.max(1, tieredPolicy.dailyDays) * 24 * 60 * 60 * 1000;
  const weeklyCutoff = now - Math.max(1, tieredPolicy.weeklyWeeks) * 7 * 24 * 60 * 60 * 1000;
  const monthlyCutoff = now - Math.max(1, tieredPolicy.monthlyMonths) * 30 * 24 * 60 * 60 * 1000;
...
export function nonSystemSchemaPredicate(identifier: string): string {
...
async function runPgDumpBackup(opts: {
  ...
  const pgDumpBin = process.env.AOA_PG_DUMP_PATH || process.env.PAPERCLIP_PG_DUMP_PATH || "pg_dump";
  const child = spawn(
    pgDumpBin,
    [
...
async function restoreWithPsql(opts: RunDatabaseRestoreOptions, connectTimeout: number): Promise<void> {
  const psqlBin = process.env.AOA_PSQL_PATH || process.env.PAPERCLIP_PSQL_PATH || "psql";
  const child = spawn(
    psqlBin,
    [
      `--dbname=${opts.connectionString}`,
