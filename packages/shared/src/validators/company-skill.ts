...
export const companySkillImportPackageSchema = z.object({
  files: z.record(z.string(), z.string())
    .refine(
      (map) => Object.keys(map).some((k) => k === "SKILL.md" || k === "skill.md"),
      { message: 'File map must include a "SKILL.md" entry' },
    )
    .refine(
      (map) => Object.keys(map).every((k) => {
        const normalized = k.replace(/\\/g, "/");
        return !normalized.split("/").includes("..");
      }),
      { message: 'File paths must not contain path traversal sequences ("..")' },
    ),
});
