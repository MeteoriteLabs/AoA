  ...
  const packageJson = {
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@armyofagents/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@armyofagents/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@armyofagents/plugin-sdk/bundlers";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@armyofagents/plugin-sdk/ui";
...
import { createTestHarness } from "@armyofagents/plugin-sdk/testing";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@armyofagents/plugin-sdk\` and \`@armyofagents/shared\` from a local Paperclip checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.paperclip-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@armyofagents/plugin-sdk/bundlers\`.
