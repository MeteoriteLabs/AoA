...
type AoaSprite = (typeof AOA_SPRITES)[number];
...
function spriteSize(sprite: AoaSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = AOA_SPRITES[(baseCol + baseRow) % AOA_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = AOA_SPRITES[Math.floor(Math.random() * AOA_SPRITES.length)]!;
