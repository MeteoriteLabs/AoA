...
describe("applyDataDirOverride", () => {
  beforeEach(() => {
    ...
    delete process.env.AOA_CONTEXT;
  ...
  it("sets AOA_HOME and isolated default config/context paths", () => {
    ...
    expect(process.env.AOA_CONTEXT).toBe(path.resolve(expectedHome, "context.json"));
    expect(process.env.AOA_INSTANCE_ID).toBe("default");
  });
  ...
  it("does not override explicit config/context settings", () => {
    process.env.AOA_CONFIG = "/env/config.json";
    ...
    expect(process.env.AOA_CONFIG).toBe("/env/config.json");
    expect(process.env.AOA_CONTEXT).toBe("/env/context.json");
  });
