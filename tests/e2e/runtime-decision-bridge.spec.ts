/**
 * runtime-decision-bridge.spec.ts — W5b Task 10 (Deliverable B)
 *
 * End-to-end acceptance spec for the full runtime-decision bridge loop:
 *   claude_local adapter → PreToolUse hook → AoA → hub item appears →
 *   founder answers allow/deny → run proceeds / blocked
 *
 ...
 *     2. AOA_E2E_RUNTIME_DECISION_BRIDGE=1  (explicit test-runner opt-in)
 ...
 *   claude_local adapter run whose PreToolUse hook hits the /internal/runtime-hooks
 *   endpoint, which requires a real logged-in `claude` CLI binary in PATH.
 *
 * Usage (local dev only):
 *   AOA_RUNTIME_DECISION_ROUTING=1 AOA_E2E_RUNTIME_DECISION_BRIDGE=1 \
 *     pnpm e2e --grep "W5b runtime-decision bridge"
 *
...
const BRIDGE_E2E_ENABLED = process.env.AOA_E2E_RUNTIME_DECISION_BRIDGE === "1";
...
const PREFIX = /^E2E-W5B-BRIDGE-/;

test.describe("W5b runtime-decision bridge — full adapter loop", () => {
  ...
  test.beforeEach(async ({}, testInfo) => {
    if (!SHOULD_RUN) {
      testInfo.skip(
        !SHOULD_RUN,
        "W5b bridge e2e skipped: set AOA_RUNTIME_DECISION_ROUTING=1 and AOA_E2E_RUNTIME_DECISION_BRIDGE=1 with a logged-in claude CLI to run",
      );
    }
  });
  ...
  /**
   * Seed a claude_local agent with runtimeDecisionRoutingEnabled=true.
   * The agent must use a real claude CLI that is logged in (so PreToolUse
   * hooks actually fire).
   */
  async function seedBridgeAgent(
  ...
  test(
    "allow_once: hook fires, hub item appears, founder allows, run proceeds",
    async ({ page, request }) => {
      const company = await seedCompany(request, `E2E-W5B-BRIDGE-${Date.now()}`);
      const agent = await seedBridgeAgent(
        request as never,
        company.id,
        "W5b Bridge Allow Agent",
      );

      // Start a run that will exercise a Bash tool — this triggers the hook
      ...
      // The run should eventually complete (the adapter relays the answer and resumes)
      // Wait for the decision status to progress past "answered"
      await expect(page.getByRole("region", { name: /runtime decision/i })).not.toContainText(
        "created",
        { timeout: 15_000 },
      );
    },
    60_000, // Long timeout: real CLI needs to start, fire the hook, and answer
  );
  ...
  test(
    "deny: hook fires, hub item appears, founder denies, adapter receives deny",
    async ({ page, request }) => {
      const company = await seedCompany(request, `E2E-W5B-BRIDGE-${Date.now()}`);
      const agent = await seedBridgeAgent(
        request as never,
        company.id,
        "W5b Bridge Deny Agent",
      );
