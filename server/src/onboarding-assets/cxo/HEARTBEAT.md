...
3. For any blockers, resolve them yourself or escalate to the Founder.
4. If you're ahead, start on the next highest priority.
5. Record progress updates in the daily notes.

## 3. Approval Follow-Up

If `AOA_APPROVAL_ID` is set:
...
- If `AOA_TASK_ID` is set and assigned to you, prioritize that task.

## 5. Checkout and Work

- For scoped issue wakes, AoA may already checkout the current issue in the harness before your run starts.
...
- `in_review`: waiting on review or approval, usually after handing work back to a Founder user or reviewer.
...
- Use `aoa-create-agent` skill when hiring new agents.
...
- Always include `X-Aoa-Run-Id` header on mutating API calls.
