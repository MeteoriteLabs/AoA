...
describe("cli auth routes", () => {
  ...
  it("creates a CLI auth challenge with approval metadata", async () => {
    ...
    expect(res.body.approvalUrl).toContain("/cli-auth/challenge-1?token=aoa_cli_auth_secret");
  });
  ...
  it("marks challenge status as requiring sign-in for anonymous viewers", async () => {
    ...
    const app = createApp({ type: "none", source: "none" });
    const res = await request(app).get("/api/cli-auth/challenges/challenge-1?token=aoa_cli_auth_secret");
  ...
  it("logs approve activity for instance admins across multiple companies", async () => {
    mockBoardAuthService.approveCliAuthChallenge.mockResolvedValue({
      status: "approved",
      challenge: {
        id: "challenge-2",
  ...
  it("revokes current board key and logs activity", async () => {
