...
const ORIGINAL_AOA_LISTEN_PORT = process.env.AOA_LISTEN_PORT;
const ORIGINAL_HOST = process.env.HOST;
const ORIGINAL_PORT = process.env.PORT;

afterEach(() => {
  if (ORIGINAL_AOA_API_URL === undefined) delete process.env.AOA_API_URL;
  else process.env.AOA_API_URL = ORIGINAL_AOA_API_URL;

  if (ORIGINAL_AOA_LISTEN_HOST === undefined) delete process.env.AOA_LISTEN_HOST;
  else process.env.AOA_LISTEN_HOST = ORIGINAL_AOA_LISTEN_HOST;

  if (ORIGINAL_AOA_LISTEN_PORT === undefined) delete process.env.AOA_LISTEN_PORT;
  else process.env.AOA_LISTEN_PORT = ORIGINAL_AOA_LISTEN_PORT;
...
describe("buildAoaEnv", () => {
  ...
  it("uses runtime listen host/port when explicit URL is not set", () => {
    ...
    process.env.AOA_LISTEN_HOST = "0.0.0.0";
    process.env.AOA_LISTEN_PORT = "3101";
