...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@armyofagents/adapter-cursor-local/server";
import { parseCursorStdoutLine } from "@armyofagents/adapter-cursor-local/ui";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$AOA_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $AOA_API_KEY\"";
