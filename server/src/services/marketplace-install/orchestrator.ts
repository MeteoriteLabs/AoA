 ...
 *
 * Plugin installs are NOT handled by `dispatchInstall` directly — pluginLoader
 * injection happens at the route layer. Routes call `runPluginInstall`
 * (defined in routes/marketplace-installs.ts) for plugin items, and
 * `dispatchInstall` for everything else.
 */
...
import type { InstallMarketplacePluginOpts, InstallMarketplacePluginResult } from "./plugin-installer.js";
...
export interface Installers {
  ...
  installMarketplacePlugin: (opts: InstallMarketplacePluginOpts) => Promise<InstallMarketplacePluginResult>;
}
...
export async function dispatchInstall(opts: DispatchInstallOpts): Promise<void> {
  ...
  try {
    ...
    if (catalogItem.type === "skill") {
      ...
      throw new Error(
        "Plugin installs are routed through pluginLoader at the route layer; " +
        "dispatchInstall plugin path requires pluginLoader injection — see route handler",
      );
    } else {
