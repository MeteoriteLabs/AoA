 ...
 * These are the SUCCESS-side composed functions the crew runner calls as ONE
 * best-effort call, so the exact SEQUENCE the runner runs is a single tested
 * unit (D4) rather than loose calls scattered in runner.ts.
 *
 * (Task 3 adds the FAILURE-side `postCrewRunFailure` here.)
 *
 * Best-effort granularity: each sub-step (relay, summary) is wrapped in its
