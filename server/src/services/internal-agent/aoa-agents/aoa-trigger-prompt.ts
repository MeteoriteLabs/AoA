...
const ROLE_ACTION_DIRECTIVE: Record<string, string> = {
  ...
  adjutant:       "Read the recent conversation first. If the founder has asked a question, raised a problem, or invited the team's input and no agent has substantively responded yet, call `post_entry` exactly once with a brief, helpful reply that moves the thread forward — an unanswered founder message is not 'nothing to do'. If the conversation has converged on concrete work to do, call `propose_crew_work` with the proposed tasks; you may also call `advance_phase` for phase transitions. If the thread is mid-work, nothing is new, or there is genuinely nothing to add, return without posting — silence is correct.",
