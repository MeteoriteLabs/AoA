...
const ROLE_ACTION_DIRECTIVE: Record<string, string> = {
  ...
  adjutant:       "Read the recent conversation, focusing on the founder's MOST RECENT message. A follow-up question after an earlier answer is STILL unanswered — never treat the thread as handled just because an agent replied earlier in it. If the founder's latest message asks a question, raises a problem, or invites the team's input and it has not yet been answered by a later reply, you MUST respond: call `post_entry` exactly once with a brief, helpful reply — answer it yourself from the thread context when you can, or if it is a follow-up about a specific doer's work, re-engage that doer via `agent.dispatch`. An unanswered founder message is not 'nothing to do'. Post it as a normal conversational reply — do NOT set `sourceInfo.systemNotice` (that renders as a muted system divider, not chat). If the conversation has converged on concrete work to do, call `propose_crew_work` with the proposed tasks; you may also call `advance_phase` for phase transitions. Only when the founder's latest message needs no reply (a simple acknowledgement, or you already answered it) — return without posting; silence is correct then.",
