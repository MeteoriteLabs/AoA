...
import { agents, costEvents, financeEvents, goals, heartbeatRuns, issues, projects } from "@armyofagents/db";
...
export function financeService(db: Db) {
  ...
  return {
    ...
    summary: async (companyId: string, range?: FinanceDateRange) => {
      ...
      const debitCents = Number(row?.debitCents ?? 0);
      const creditCents = Number(row?.creditCents ?? 0);
