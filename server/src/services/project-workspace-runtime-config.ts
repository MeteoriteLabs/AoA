import type { ProjectWorkspaceRuntimeConfig } from "@armyofagents/shared";
...
export function readProjectWorkspaceRuntimeConfig(
  ...
  const raw = isRecord(metadata?.runtimeConfig) ? (metadata.runtimeConfig as Record<string, unknown>) : null;
  if (!raw) return null;
