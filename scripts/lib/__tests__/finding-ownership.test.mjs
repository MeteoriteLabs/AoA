...
test("★ owned by a ticket that has ALREADY SHIPPED fails — owned by nothing", () => {
  ...
  // still open, and nothing was checking.
  const r = evaluateFindingOwnership({
    findings: [OPEN_HIGH],
  ...
  assert.equal(r.ok, false);
  assert.deepEqual(kinds(r), ["owner_ticket_already_complete"]);
...
test("★ naming a part-shipped ticket is allowed ONLY by writing what is still open", () => {
  ...
  const declared = (extra) => ({
    "E4-F007": { status: "owned", ticket: "WRK-008", reason: "lands in the live-dispatch wiring", ...extra },
  });
  const args = { findings: [OPEN_HIGH], ticketIds: ["WRK-008"], completedTicketIds: ["WRK-008"] };
