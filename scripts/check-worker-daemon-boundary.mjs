#!/usr/bin/env node
/**
 * check-worker-daemon-boundary.mjs
 *
 * Always-on `policy`-job guard for the `@armyofagents/worker-daemon` leaf
 * package (E4-D01). It enforces that the package declares EXACTLY the runtime
 * dependencies `@armyofagents/worker-protocol` + `pino`, and that its runtime
 * source imports ONLY the worker protocol, `pino`, Node built-ins, or relative
 * modules that stay inside the package `src` directory — no `@armyofagents/db`,
 * `@armyofagents/server`, `@armyofagents/shared`, `@armyofagents/adapters`,
 * `drizzle-orm`, `pg`, Express, or any other bare package. Alternate
 * runtime-source extensions and symlinks under `src` are rejected too.
 *
 ...
 * `scripts/lib/worker-daemon-boundary.mjs`.
 *
 ...
 *   node scripts/check-worker-daemon-boundary.mjs --root <fixture-dir>
 *
 * `--root` exists only for the dependency-free `node:test` harness in
 * `check-worker-daemon-boundary.test.mjs`; it defaults to `process.cwd()` in
 * normal and CI use.
 */
...
} from "./lib/worker-daemon-boundary.mjs";

const PACKAGE_REL = "packages/worker-daemon";
...
export async function runBoundaryCheck(root, deps = {}) {
  ...
  const packageRoot = path.join(root, "packages", "worker-daemon");
...
async function main() {
  ...
  console.log("worker daemon boundary: PASS");
}
