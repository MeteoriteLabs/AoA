...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$OPENCLAW_TMP_DIR/openclaw-aoa-smoke}"
...
AOA_HOST_PORT="${AOA_HOST_PORT:-3100}"
AOA_HOST_FROM_CONTAINER="${AOA_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.aoa-openclaw.override.yml"
...
detect_paperclip_base_url() {
  ...
  for candidate in "$AOA_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    ...
    health_url="http://${candidate}:${AOA_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${AOA_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${AOA_HOST_PORT}
  (Do not use http://127.0.0.1:${AOA_HOST_PORT} inside the container.)
  If AoA rejects the host, run on host machine:
    pnpm aoa allowed-hostname host.docker.internal
  Then restart AoA and re-run this script.
EOF
  fi
  cat <<EOF
