...
**Deferred:** the real multi-worker `GvisorPoolClient` implementation + worker-token rotation (Task 13 ships id-scoped heartbeat auth using the target's own primary key as a bearer credential, a deliberately migration-free stopgap); the Task 0 hardware spike itself (Gate-B, pending); generalizing the egress firewall beyond the documented iptables/nftables sketch to a maintained, tested artifact.
