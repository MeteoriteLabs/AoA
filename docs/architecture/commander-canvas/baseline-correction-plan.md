...
- Codex authors implementation/evidence only after explicit scope approval; TK owns acceptance; TK-managed Claude independently reviews material technical plans/changes. Do not assign another replatform maintainer by assumption.
...
**Not yet performed:** source edits, branch creation, bundle/checkout setup, targeted regressions, another full suite/build, Claude review of this plan, base integration or E1.1 implementation. The code sketches are uncompiled proposals. Current deliverable: source-checked, self-reviewed corrective plan ready for independent review and explicit execution-scope approval.
