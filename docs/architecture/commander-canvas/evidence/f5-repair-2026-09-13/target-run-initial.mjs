...
const cmds={red:unit,green:unit,negativeActive:unit,negativeWait:unit,finalUnit:unit,typecheck:['corepack','pnpm','--filter','@armyofagents/db','typecheck'],integration:['corepack','pnpm','exec','vitest','run','packages/db/src/__tests__/backup-lib-non-system-schemas.test.ts','server/src/__tests__/blocked-task-fixture.test.ts']};
...
if(label!=='typecheck')cmd.push('--reporter=default','--reporter=json','--outputFile.json='+dir+'/'+label+'-tests.json');
const rec={label,sha,command:cmd,startedAt:new Date().toISOString(),beforeHashes:Object.fromEntries(allowed.filter(p=>before[p]).map(p=>[p,before[p]]))};
...
rec.expectedRed=['red','negativeActive','negativeWait'].includes(label);
