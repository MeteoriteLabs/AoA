...
| `AOA_APP_DATABASE_URL` | unset | Required only when distributed execution is enabled. PostgreSQL URL authenticating exactly as the NOSUPERUSER/NOBYPASSRLS `aoa_app` tenant-serving role. Missing, invalid, privileged, or wrong-role credentials fail startup; there is no owner-pool fallback. |
| `AOA_OPERATOR_DATABASE_URL` | unset | Required only when distributed execution is enabled. PostgreSQL URL authenticating exactly as the bounded NOSUPERUSER/NOBYPASSRLS `aoa_operator` platform-metadata role. It is not a tenant/job-data connection. |
