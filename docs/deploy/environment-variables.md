...
| `AOA_WORKER_ENV_PROBE` | unset (off) | DEP-017. Runs the live env-absence probe inside every distributed sandbox after stage-in, with the run's own env, plus a planted-canary positive control; any present credential class, a probe that did not run, or a blind control fails the attempt closed. Reports credential classes and env-var NAMES only, never values, as one `system` log event on the attempt. Also records (never enforces) whether `169.254.169.254` answers (the DE-08 residual). Exactly `1` enables; unset/empty/`0` disable; any other value is a startup error. Set only by the DEP-015 shipped-boot overlay (`docker/m1-boot/docker-compose.m1-boot.yml`), which a static invariant requires on every shipped-boot worker. |
| `AOA_WORKER_RUN_OUTPUT_PROBE` | unset (off) | DEP-023, armed on the shipped-boot lane by DEP-024. Forwards AT MOST ONE tagged (`AOA-RUN-OUTPUT-PROBE`), truncated (512 chars) line of the **already-scrubbed** run output to two surfaces: a `log` event on the attempt and one worker log line. It is an observation surface for redaction, NOT an output mechanism — only a line a workload explicitly tagged is ever selected, and the run’s result line is not. Nothing unscrubbed can ride it: the input is `createRunOutputCapture.close()`’s fail-closed return (any surviving needle drops the WHOLE tail), the event half is scrubbed over the complete envelope in `EventSequencer.#emit`, and the log half is scrubbed at the pino **destination**, below the `msg`/`time`/`level` the sink adds (`E4-F019` closure route 2). Exactly `1` enables; unset/empty/`0` disable; any other value is a startup error. Set only by the D1 fault-matrix override (`docker/d1/m1-spine.override.yml`) and the DEP-015 shipped-boot overlay (`docker/m1-boot/docker-compose.m1-boot.yml`), which a static invariant requires on every shipped-boot worker. |
...
| `AOA_WORKER_LEASE_CANDIDATE_PATH` | beside the outbox: `<AOA_WORKER_EVENT_OUTBOX_PATH minus .db>.lease-candidates.db` | WRK-013. Filesystem path for the daemon's durable **lease-candidate store** (a SQLite DB): a row is written just before a lease is ACKed, withdrawn if the ACK fails, and removed when that attempt ends in the same process, so a row that survives a restart is a lease the daemon held when it stopped. At the next start, before the first poll, the startup reconciler probes each such lease once and **fences** a live one (founder ruling F5: that probe's renewal is its last; the control-plane reaper ends the attempt). Unset/empty/whitespace ⇒ the default beside the outbox, on the same writable volume, so no deployment change is needed. An unreadable file is renamed aside (`<path>.corrupt-<ms>`), a fresh store is opened, and **no** lease it held is renewed. If no store can be opened at all, the daemon keeps polling but **ACKs no lease** (it will not take work it cannot record). Read only inside the `compose:true` branch. |
