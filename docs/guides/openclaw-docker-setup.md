...
How to get OpenClaw running in a Docker container for local development and testing the AoA OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

AoA includes an end-to-end join smoke harness:
...
```bash
AOA_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
AOA_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```
