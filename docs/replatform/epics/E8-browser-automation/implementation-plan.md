...
| `BRW-007` — agent-facing session request — ★ **OUT of M3 scope; needs a programme-owner decision before it is chartered at all** | **no ticket file, no program-design node**; scope only in `scope-addendum-agent-and-commander.md:46` | Needs BRW-002 + BRW-004 + BRW-006. The `ask_human` precedent is the named authorization model. |
| `BRW-008` — Commander on the governed path; retire the host spawn — ★ **OUT of M3 scope; needs a programme-owner decision before it is chartered at all** | **no ticket file, no program-design node**; scope in `scope-addendum-agent-and-commander.md:67` | Its removal proof already exists (`scripts/browser-spawn-expectation.json`). Closing the spawn is a **decrement of the declared expectation**, and the guard must stay red-when-broken across it. |
...
5. **`JOB-015` lands the control-command delivery hop** — `E8-F004` repoints to it and
