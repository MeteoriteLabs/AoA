...
| E11 scope inside M4 | `REL-003`'s owed DR rehearsal with measured RPO/RTO, and `DBR-001` (`scope-triage.md`), alongside the non-E11 items **the production-scale HA bar, owned by D5**, and `WRK-016`. ★ *Corrected 2026-09-20 (fifth round): this named “`DEP-009`'s two-replica half”, but the companion change moved `DEP-009` wholly to disposition **A** — its two-replica half shipped (`complete + CI-GREEN`, `e6f-11` 6/6, `control-plane-b` in the D1 compose). Naming it here would have told the M4 planner to re-open an approved ticket instead of the real D5 owner. `DEP-009` remains prerequisite EVIDENCE, not M4 work.* |
...
| `REL-004` lanes A–D | signed-artifact and kill-switch mechanism | test signing root only; the gate is not on the publish path. ★ *“No write path for the switch” is **superseded** — authorized `PUT`/`DELETE` ship and are actor-attributed (`server/src/routes/instance-settings.ts:92,108,131`). The `REL-004` record is **frozen** and is corrected here by finding, never edited; the surviving limitation is the **UI** and the drain-integration decision.* |
...
|---|---|---|---|
| `REL-001` — tenant/secret adversarial + load/fairness release tests | **zero files** | M5 | **Write at Step 0 of M5.** Its deferral row in `docs/architecture/distributed-execution-release-tests.json` is removed in the same commit that adds `REL-001-design.md`. Also the natural home for the kill-switch **UI and drain-integration decision** (B3) unless a founder assigns it elsewhere. ★★★ **NOT the write path — that SHIPS** (`server/src/routes/instance-settings.ts:92` authorized `PUT`, `:131` `DELETE`, actor-attributed via `logActivity` at `:108`). *Corrected fourteenth round: this task row still scheduled an API that exists, which would have had M5 build a duplicate operator surface.* |
