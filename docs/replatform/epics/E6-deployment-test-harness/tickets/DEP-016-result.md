...
| The verdicts' self-test | `scripts/lib/__tests__/m1-spine-assertions.test.mjs` (42 tests) | Each verdict has a zero-violation anchor and defect fixtures. Wired into `pr.yml` `policy` → *m1-spine profile verdict self-test (DEP-016)*, declared in `scripts/test-execution-census.json`. |
...
- `scripts/lib/__tests__/m1-spine-assertions.test.mjs`: **42/42**.
