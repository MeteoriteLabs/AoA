...
**Status:** `complete`. The **keyed acceptance is PENDING**: it needs one dispatched keyed run of
...
| 3 | M1a closeout independent reviewer (Codex) | `446d99f39c8f8f9bece2138252fe48c839fbc7a6` | `approved`; `Status` set to `complete` | Successful keyed run `36187103782` closes both items left open by attempt 2 on this exact candidate. Retained `env-probe-a.json` and `env-probe-b.json` bind the probe to each enabled tenant's real distributed attempt; each reports verdict `absent`, the complete 18-class checked set, empty `present`/`presentNames` and `allowedMismatch`, and red planted controls for datastore credential, cross-tenant credential, and provider-value mismatch. `journey.json` binds tenant A to organization `6a6ff0a1-b79e-4878-b4d5-231abc0a3808` / attempt `485147a7-fb39-4605-a34b-e74c21378fef`, and tenant B to organization `4651d76b-e0a2-4cb5-97d1-61fca6957e0c` / attempt `23e33a4f-9a2f-4bf1-895b-451815319817`. Both probes record DE-08 as attempted and reachable with HTTP 401, explicitly `OBSERVED, not enforced`, so this disposition makes no egress-enforcement claim. The merged `M1a-D2-MECHANISM` attempt-25 record (blob `af14519fba6e709d5333027ad293318e72fdf102`) and E5 exit-audit attempt-3 record (blob `04f68276825896b9f6f36f44b5b17369fa5a02cb`) retain the same campaign-level conclusion. The opening pending sentence is preserved as historical text and superseded by this append-only review. Existing non-blocking defects and findings remain unchanged. |
