...
`env-probe.test.ts` **61 passed** and the whole-package **1220 passed** are taken from CI, not
re-run by me. That is stated rather than implied.
...
**Acceptance, restated against the E6 plan's `### DEP-017` and its §3 verify row.** Acceptance 1–4
(absence report naming every class checked; the planted-canary positive control; the per-run canary
redaction; multi-tenant with another tenant's credential planted) are **evidenced keyless at the
tip**. The verify row's Linux/CI half — the probe **observed inside the `DEP-015` lane** — is
**OPEN**. `complete` requires every acceptance item, so this ticket stays `gate_review` and a later
attempt must re-review it once the keyed run is recorded per §7.
