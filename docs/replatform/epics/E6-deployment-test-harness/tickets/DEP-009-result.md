...
**CI-caught residual (HIGH-3):** the first push (`dac459558`) still crash-looped both replicas — the review's HIGH-1 fix registered the table in the keystone *manifests* but missed the **second, independent registration surface**: `appTablePrivileges()` in `distributed-execution-databases.ts` (the map `assertExactServingRoleAuthority` actually reads). `d1-merge-train` caught it (`distributed_execution_app_authority` at boot); fixed + a new **local** cross-surface guard added (see HIGH-3 below).
...
| 2 | `d1-merge-train` CI (DEC-03 authority) + controller diagnosis | first push `dac459558` | `rejected → fixed` | HIGH-3: both replicas crash-looped `distributed_execution_app_authority` — HIGH-1's manifest registration missed the second `appTablePrivileges()` grant surface. Diagnosed from the evidence bundle, fixed (import+spread), and closed the class with a local cross-surface guard proven RED→GREEN. |
