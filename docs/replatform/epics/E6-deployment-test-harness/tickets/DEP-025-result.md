...
| The same for the **SUPPRESSED** arm | ★ **FALSE, and not claimed.** A suppressed-arm failure clears only `positiveControlPassed`, which the grader's redaction branch never reads (§11), so an artifact-only verdict still passes on that half. An earlier draft of this very table claimed it for "either arm"; Codex caught the overclaim against my own §11 and it is corrected rather than softened |
