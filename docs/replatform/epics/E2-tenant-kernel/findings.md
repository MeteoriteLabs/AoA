...
- **Blocks gate:** **Yes** (TEN-006a acceptance: "exhaustive Company-writer sweep"; the mandatory E2-D05/E2-F008 Linux H-01 run would go RED across these files).
...
- **Disposition:** **Open — changes_requested.** Sweep every `companyService(db).create(…)` / `createWithOperator(…)` test site to pass `organizationId` explicitly (literal sentinel to match the raw-SQL sweep, or route via the `insertTestCompany` factory), reconcile the ledger §5 inventory to the true count, and re-run the affected integration files on embedded PG (`AOA_RUN_WIN_INTEGRATION=1`) to GREEN before re-review. The intentional throw-assertions in `company-writer-fail-closed.test.ts` (the 2 no-org cases) are correct and must remain org-less.
- **Resolution link:** [decisions.md#E2-D07](decisions.md); TEN-006a-result.md.
