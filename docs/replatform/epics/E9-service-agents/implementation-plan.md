...
| **SVC-005b** — the `drain` producer | ★★★ **THE PRODUCER SHIPS; what is missing is the RESULT DOC.** *Corrected twelfth round:* `requestDrain` persists a `drain` command and `job-reconciliation.ts` calls it, so “design only” understates delivery. Residual: the result record, plus graceful-stop and checkpoint. Superseded text: **DESIGN ONLY**, no result (`tickets/SVC-005b-design.md:3`) | Would close the `drain` third of `E9-F008`; explicitly **does not** close `E9-F008` (`graceful_stop` and `checkpoint` stay zero-producer). |
