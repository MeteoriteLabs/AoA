...
| **SVC-005b** — the `drain` producer | ★★★ **THE PRODUCER SHIPS; what is missing is the RESULT DOC.** *Corrected twelfth round:* `requestDrain` persists a `drain` command and `job-reconciliation.ts` calls it, so “design only” understates delivery. Residual: the result record, plus graceful-stop and checkpoint. Superseded text: **DESIGN ONLY**, no result (`tickets/SVC-005b-design.md:3`) | **Closes the `drain` third of `E9-F008` already** — the producer ships (`requestDrain`, `packages/db/src/repositories/tenant/job-control.ts:5145`, reached from `server/src/services/job-reconciliation.ts:134`). It does **not** close `E9-F008`: `graceful_stop` and `checkpoint` stay zero-producer. ★ Superseded text: *“Would close the `drain` third of `E9-F008`.”* |
...
| every control action is audited | **MOSTLY DELIVERED** — ★★★ *corrected 2026-09-20 (ninth round), verified at source.* **Three durable writers ship**: `recordServiceGenerationRollActivity` (`server/src/services/service-generation-rollout.ts:415`), `recordJobSubmitActivity` (`server/src/services/job-submission.ts:406`) and `recordJobDrainActivity` (`server/src/services/job-reconciliation.ts:151`) — so **roll, job submission and drain DO write durably**, and this row's earlier claim that the roll route “writes nothing durable” was false. The surviving gap is worker **revoke**, and it is **BLOCKED** on its documented organization/company-scope problem — not merely unwired. ★ Superseded text, retained for the record: *“the roll route writes nothing durable; roll, job submission, `drain` and worker `revoke` are all missing their audits.”* |
| generation update | ★ **MOVED SINCE THE README's SVC-007a ENTRY.** `README.md:9` lists *"nothing rolls a generation (SVC-005)"* as not-delivered; SVC-005a subsequently shipped `rollServiceGeneration`. ★★★ *Corrected, verified at source: the route-level audit for it is NOT missing.* `rollServiceGenerationWithinTenant` writes `recordServiceGenerationRollActivity` **inside the same transaction as the mint and the generation bump** (`server/src/services/service-generation-rollout.ts:415`), so a generation change is durably audited exactly when a generation changed. ★ Superseded text: *“The **route-level** audit for it is still missing (above).”* |
...
| **★ the largest** | ★★★ **PARTLY DELIVERED — corrected twelfth round.** `server/src/__tests__/service-leased-supervised.integration.test.ts` polls, asserts a `service` workload, ACKs, checks persisted lease state and invokes the real supervisor. The **real** residuals are seeded placement, a mock provider, locally collected events with no server ingestion, and insufficient duration. ★ Superseded claim, retained in full: *“no service job is leased anywhere in its suite — the DAEMON half of ‘created, supervised, projected’ is unexercised.”* See §5.1. |
...
| Frozen control-command vocabulary | `packages/worker-protocol/src/transport.ts:601-608` — `CONTROL_COMMAND_KINDS` (six kinds) | ★★★ *Corrected, verified at source:* **four** of the six are produced — `drain` among them, via `requestDrain` (`packages/db/src/repositories/tenant/job-control.ts:5145`), reached in production from `server/src/services/job-reconciliation.ts:134`. Only `graceful_stop` and `checkpoint` have zero producers (`E9-F008`); `checkpoint` additionally cannot be persisted (§5.2). ★ Superseded text: *“Three are produced; `graceful_stop`, `drain`, `checkpoint` are not.”* |
...
| **`E9-F008`** | MED · `unowned` | ★★★ *Corrected twelfth round: **`drain` now HAS a producer** — `requestDrain` (`packages/db/src/repositories/tenant/job-control.ts`) called from `server/src/services/job-reconciliation.ts`. The residual is **two** kinds, `graceful_stop` and `checkpoint`.* Superseded text: Three of six frozen control-command kinds (`graceful_stop`, `drain`, `checkpoint`) have **zero producers**; `checkpoint` additionally cannot be persisted (§5.2); and a repository docstring said otherwise. Not a one-line gap — `queueGovernedControlCommand` is narrowed at the **type** level. **Blocks SVC-003's graceful-stop and checkpoint conjuncts and SVC-005's drain conjunct.** `SVC-005b` is designed for the `drain` third only. |
...
| **`E9-F010`** | MED · `unowned` (**half resolved**) | Both mutating *service* routes now write `activity_log`. ★★★ **Still open on `revoke` ONLY — corrected twelfth round, verified at source.** `roll`, job submission and `drain` all write durably today: `recordServiceGenerationRollActivity` (`server/src/services/service-generation-rollout.ts:415`), `recordJobSubmitActivity` (`server/src/services/job-submission.ts:406`) and `recordJobDrainActivity` (`server/src/services/job-reconciliation.ts:151`), which is also what this epic's own `findings.md` records (*“revoke-only — BLOCKED”*). `revoke` is **blocked**, not merely unwired, on its organization/company-scope problem. Superseded text: Still open on the **roll** route, plus job submission, `drain` and worker `revoke` — unwired rather than blocked. `jobAuditBridge` still has zero production callers. Note DE-01 is untouched: its `audit` clause is about **policy-denial** events, not mutating actions. Blocks SVC-007's "every control action is audited". |
...
| **`E9-F012`** | **HIGH** · `unowned` | Substantively **resolved by `E9-D001` route (c)**; the register close is **pending the custodian** (§4). It does not block the gate — E9's gate is unmet for larger reasons — but it bounds what SVC-005a may be read to claim. DE-12's `deliveryStatus` stays `partial` for the register-coupling reason in §4. ★★★ *Corrected, verified at source:* the generation change **is** durably recorded — `recordServiceGenerationRollActivity` commits in the roll's own transaction (`server/src/services/service-generation-rollout.ts:415`); the route's `logger.info` line is process telemetry **beside** that row, not in place of it. ★ Superseded text: *“a `logger.info` line is not a durable record.”* |
...
- ★★★ **CORRECTED 2026-09-20 (ninth round) — A SERVICE-LEASING PROOF DOES EXIST.**
  ...
  *Superseded claim, retained for the record:* “No service job is leased anywhere in any E9 suite
  … the daemon half of ‘created, supervised, projected’ is unexercised end to end”
  (`README.md:9`, `:11`). The residual coverage gap named above is distinct from
...
| `SVC-005` residual | OPEN on hard limits and force-kill; **drain only PARTLY** — ★ *corrected: the disposition cell previously read “OPEN on drain”* | ★★★ **the drain PRODUCER ships** (`requestDrain`, `packages/db/src/repositories/tenant/job-control.ts:5145`, called from `server/src/services/job-reconciliation.ts:134`) — *the residuals are graceful-stop and the checkpoint, not the producer.* `SVC-005b` is designed for the `drain` third only, and its own residual is the **result doc**. Decide whether it is revived as-is or folded into a whole-`E9-F008` unit. Replace-before-stop needs a decision, not a task. |
| `SVC-007` residual | OPEN on audit-completeness (**worker `revoke` only**), the view, idempotent create, realtime | ★ Audit-completeness is down to worker `revoke`, which is **BLOCKED** on its organization/company-scope problem rather than unwired; roll, job submission and `drain` all write durably today (§2.3, §5.3). The view's three missing columns (checkpoint, budget, restart history) depend on SVC-004/SVC-005 existing at all. `E10-REALTIME-FOUNDATION` must be confirmed passed. |
...
5. **Either of `graceful_stop` / `checkpoint` gains a producer** (`E9-F008`) — that releases
   SVC-003's blocked conjuncts. ★★★ *Corrected, verified at source: `drain` is struck from this
   trigger because it is ALREADY satisfied* — `requestDrain`
   (`packages/db/src/repositories/tenant/job-control.ts:5145`) is reached in production from
   `server/src/services/job-reconciliation.ts:134`, so a trigger waiting on a `drain` producer
   could never fire. `SVC-005b`'s residual is its **result doc**, not its producer, so “`SVC-005b`
   ships” is likewise not a reopen condition for SVC-005's drain conjunct. ★ Superseded text:
   *“Any of `graceful_stop` / `drain` / `checkpoint` gains a producer (`E9-F008`), or `SVC-005b`
   ships — that releases SVC-003's and SVC-005's blocked conjuncts.”*
