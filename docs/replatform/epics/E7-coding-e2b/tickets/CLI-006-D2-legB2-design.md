...
| **no-leak (Decision #104)** | the redeemed value is returned ONLY in `env` + the `canary` array; the audit is columns-only | `job_secret_handles.resolve_count` incremented (audit-as-columns, no value column); the seeded plaintext appears in NO log line the harness captures |
...
| A4 | The value is contained — audit-as-columns only, no log leak (Decision #104) | `resolve_count` incremented; no seeded plaintext in captured logs | embedded-PG |
