...
This README is not evidence. It indexes the records and holds the frozen audit plan below.
...
## The frozen audit plan (`E5-A2-MATRIX`)

**Frozen by:** ticket `E5-A2-MATRIX` ([`../tickets/E5-A2-MATRIX-result.md`](../tickets/E5-A2-MATRIX-result.md)).
It takes effect when a distinct reviewer approves that ticket. The result records this file's git
blob SHA at freeze. Every attempt from `a2` onward cites that SHA. If this section's blob no longer
matches, the plan changed after the freeze. That change needs its own recorded decision before any
attempt may rely on it.
...
**Retained full-gate non-certifications.** Every attempt restates each of these, whatever its grades:
...
| This plan unchanged since freeze | `git rev-parse <candidate sha>:docs/replatform/epics/E5-workspaces-secrets/qa/README.md` | equals the blob SHA recorded in `E5-A2-MATRIX-result.md`, **or** the attempt cites the recorded decision that changed it |
