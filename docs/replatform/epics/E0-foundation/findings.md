...
| ~~DE-20 (Critical)~~ — **AUDIT CLAUSE RESOLVED BY AMENDMENT 2026-09-11 (Decision 1); STRUCK FROM THIS COHORT** | "cutover selection and rollback transitions are audited" | `buildCutoverSelectionEvent` append at `server/src/services/heartbeat.ts:5388`, before the `shouldSuppressLegacyExecution` gate (cited by symbol; the older `:5399`→`:5451` line numbers were stale). | **RESOLVED BY AMENDMENT (Decision 1).** 4a (cutover selection): DELIVERED for BOTH arms — one unconditional append at heartbeat.ts:5388 (buildCutoverSelectionEvent, TOTAL over RunExecutionOwner, W20-B 2026-09-10) writes a distributed_execution_selection row for distributed AND legacy. 4b (rollback transitions): AMENDED as VACUOUS — createDistributedExecutionDrain (job-distributed-drain.ts:114) has zero production callers, so no rollback transition occurs. Audit clause resolved; DE-20 leaves this cohort and stays `partial` in the register (its revocation clause still has no chartered exit — E0-F014/REL-005). ★ **Corrected 2026-09-23 (record custodian), fact only, no disposition change:** the 4b premise *"`createDistributedExecutionDrain` … has zero production callers"* is FALSE at HEAD — `MIG-009` (PR #544) added `runDistributedExecutionDrainTrigger` (`server/src/services/distributed-execution-drain-trigger.ts`), the one production caller, reached from `server/src/cli/drain-distributed-execution.ts`, and each of its cancels writes `job.drain.requested` in the same tenant transaction (`createAuditedDrainCancellation`, `E10-D002`). **This row's disposition, DE-20's status and its clauses all stand unchanged for M1** by binding planning ruling (recorded on DE-20 in `docs/architecture/distributed-execution-threat-controls.json`); re-examining the dropped conjunct is deferred to post-M1. The 2026-09-11 text is kept as written. |
...
- **DE-20's rollback half** — ~~`createDistributedExecutionDrain` has **zero production callers**
  ...
  ★ **Corrected 2026-09-23 (record custodian), verified at source: the zero-caller premise is FALSE
  at HEAD.** `MIG-009` (PR #544) built the operator trigger `runDistributedExecutionDrainTrigger`
  (`server/src/services/distributed-execution-drain-trigger.ts`), invoked from the shipped CLI
  `server/src/cli/drain-distributed-execution.ts` (`pnpm drain:distributed-execution --operator
  <who>`), and it calls `createDistributedExecutionDrain(...).drainAll(...)`. That is **one
  production caller**, so a rollback transition now occurs and is no longer prose. Each cancel it
  performs is audited as `job.drain.requested` in the **same tenant transaction** as the cancel
  (`createAuditedDrainCancellation`; E10 decision `E10-D002`), which is the atomicity this register
  demands of an audited mutation. **Nothing about DE-20's status or clauses changes here** — see the
  binding planning ruling recorded against DE-20 in
  `docs/architecture/distributed-execution-threat-controls.json`: the question of whether an
  operator-run, audited fleet rollback revives the *"rollback transitions are audited"* conjunct that
  Decision 1 dropped as vacuous is **deferred to post-M1** and is not reopened during M1. The struck
  text is kept above as the record of what this bullet said.
...
## E0-F019 — `check-finding-ownership` validates only the JSON register, never the `Owner:` line a reader actually reads

**Status:** open · **Severity:** LOW · **Owner:** `unowned`
**Filed:** 2026-09-21, M0 exit review (independent review of the eight M0 `gate_review` records).

**What happened.** `E7-F007`'s prose line in `epics/E7-coding-e2b/findings.md` read
`**Owner:** MIG-010 (… no result doc)` and *"MIG-010 owns choosing"*, while
`scripts/finding-ownership.json` had already been re-pointed to `MIG-011` and `MIG-010-result.md` had
been committed. `check-finding-ownership` **passed** throughout, because ownership is
**declaration-based**: the guard reads the JSON register (`scripts/lib/finding-ownership.mjs`) and
never the findings prose. So a finding's human-readable owner can silently disagree with its
...
`**Owner:**` line and require it to match the JSON entry; (2) make the prose owner a generated field
so there is one source; (3) accept the gap and record that the prose owner is non-authoritative.
...
### Addendum (2026-09-24) — a SECOND hole in the same guard: mnemonic decision ids are never scanned

Recorded by the M1 planning session as custodian, after two `E6-F` id collisions in one day
(`E6-F026` minted twice, then `E6-F027` minted twice). It is an addendum rather than a new finding
because it is the same guard being partial in a second way, and a reader who fixes only the half
above will still be exposed.

**What.** `EPIC_DECISION_HEADING` (`scripts/lib/register-id-uniqueness.mjs`) is
`/^#{2,4}\s+([A-Z][A-Z0-9]*-D\d+)(?=\s|$)/` — it requires `-D` followed by **digits**. Measured across
`docs/replatform/epics/*/decisions.md` at this revision: **31 decision ids are scanned and 4 are
not** — `E3-D-ACC`, `E3-D-AUDIT-SET`, `E3-D-OUTPUT-MAP`, `E3-D-TERMINAL-WINNER`. Every mnemonic id is
invisible to the uniqueness check, so two branches may mint the same one with nothing objecting.

**Why it compounds the finding above.** `E0-F021` records that the numeric registers are checked
only when **both** copies survive a merge textually. The mnemonic ids are not checked **at all**,
in either direction. The weaker case is the one nobody is watching.

★ **The ruling on `E3-F041` sits behind this exact hole.** `E3-D-GEN-INVALIDATION` — the decision
governing a HIGH production defect — is itself a mnemonic id, so it is unscanned. This was
volunteered by the agent that minted it, against its own interest, and it is the sharpest statement
of the problem: the convention that documents the gap is written in the notation the gap covers.

**Not a naming quibble, and it was nearly dismissed as one.** The agent that measured it judged it
too small to file when looking at it in isolation — correct for that evidence, wrong for the
evidence that exists, since two collisions had already occurred that day. Its own diagnosis: the
same measure-the-link-not-the-chain error, with the chain here being the register's history rather
than a code path.

**Resolving it** (either is acceptable, and neither is authorised here): widen the pattern to accept
mnemonic ids and scan them for uniqueness alongside the numeric ones; or rule the mnemonic form
out of the convention and migrate the five. Whichever is chosen needs a positive control that reds
on a duplicated **mnemonic** id — the control the guard does not have today.
