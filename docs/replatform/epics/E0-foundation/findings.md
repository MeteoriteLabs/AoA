...
| DE-16 (Critical) | "blocked plugin routes, dispatch, and reconciliations are audited" | `server/src/services/cloud-plugin-execution.ts:254`; the 503 helper at `server/src/routes/plugins.ts:381`. | **★ PARTLY DELIVERED (updated 2026-09-13), a THREE-part conjunction.** (i) **Routes + BOARD dispatch — DELIVERED** (Decision 3.2, 2026-09-11): the eight `companyId`-bearing 503 route sinks (incl. the board `/plugins/tools/execute` dispatch route + the rollback) write a bounded, tenantless operator-sink row via `recordCloudPluginDenial` → `security.denied.cloud_plugin_execution` (`server/src/services/cloud-plugin-denial-audit.ts`), `company_id` NULL, caller-supplied company in `entity_id`. **★ (i-GAP) the MCP AGENT dispatch path is NOT audited** — `dispatchPluginToolCall` (`server/src/mcp/tools/plugin-broker-tools.ts:176`, sole prod caller `server/src/mcp/server.ts:689`) returns `forbidden` on the cloud block and writes no row; distinct shape (real FK-valid company + verified agent run), filed as a follow-on below. So "dispatch" is covered only for the board route. (ii) **Reconciliations — DELIVERED 2026-09-13**: after a successful flip, `reconcileCloudBlockedPlugins` (`server/src/services/plugin-lifecycle.ts:136`) calls `recordCloudPluginReconcileToBlocked` (`server/src/services/cloud-plugin-reconcile-audit.ts`), writing ONE durable `activity_log` row per reconciled plugin — real-tenant `company_id`, `actor_type="system"`, `actor_id="cloud-plugin-boot-reconciliation"`, action `security.reconcile.cloud_plugin_blocked` (reserved, NON-denial namespace), `details.priorStatus`. The idempotent-skip branch records nothing (no duplicate on re-boot). Proven RED-first by `server/src/__tests__/de-16-reconciliation-audit.integration.test.ts`. The prior "no row is written anywhere / process-memory counter resets on restart" (`recordCloudPluginBlock`) is now true only of the lifecycle-activation block sink and the i-GAP path. DE-16 stays `partial` (i-GAP unaudited; TTL/confidentiality unknown; REL-005 zero files). |
