...
| DE-04 (Critical) | "claim, fence-generation, and stale-claim rejection are audited" | `packages/db/src/repositories/tenant/job-control.ts:1167` / `:1177` / `:1185` / `:1187` (`guardActiveFence`, 11+ governed mutators call it). | **Nothing.** The one table that looks like it records rejections, `worker_lease_rejections`, is an eligibility-certificate cache whose own header excludes exactly this class — *"Dynamic capacity, liveness, lock, parsing, and **authority failures** are deliberately excluded"* (`packages/db/src/schema/worker_lease_rejections.ts:16-18`), and its single writer (`job-control.ts:2127`) inserts placement certificates, not fence refusals. **★ NARROWED 2026-09-13 by the DE-04+DE-18 fence-denial-audit unit:** the WORKER-reachable governed-fence refusals are NO LONGER silent — every `guardActiveFence` deny (the current throws are `job-control.ts:1956`/`:1966`/`:1974`/`:1976`, not the stale `:1167…` above) reached through a worker-facing owner (`renewLease`, `acceptEvent`, `completeAttempt`, `ackControlCommand`, `resolveExecutionSecret`, `recordPatchApplyState`) now writes ONE attributable `security.denied.fence_guard` `activity_log` row via `recordFenceGuardDenial` (`server/src/services/fence-denial-audit.ts`) — `actorId=workerId` (never the tenant), company+org scoped, crossing keyed by reason (`stale_fence`/`attempt_terminal`→DE-04) — drained on the POOL handle after the tenant tx unwinds; proven RED-first against real PostgreSQL by `server/src/__tests__/de-04-18-fence-denial-audit.integration.test.ts`. The control-plane governance-bridge `guardActiveFence` denies (`recordGovernedProjection`/`lockActiveFence`/`queueGovernedControlCommand`/`markGovernedProjectionApplied`) remain a follow-on, and the artifact-commit/transfer fence throws were already recorded under DE-06's `security.denied.artifact_commit`/`…_transfer_grant` surface. The DE-04 audit gap is thereby NARROWED, but the formal audit-conjunct re-grade + cohort/count re-derivation is DEFERRED to a dedicated pass (to avoid an unreviewed count cascade, cf. the DE-13 row). DE-04 stays `partial` and is STILL listed in this cohort pending that pass. (No count or cohort change is taken here.) |
...
| DE-18 (Critical) | "placement decisions and generation changes are audited" | `packages/db/src/repositories/tenant/job-control.ts:1177` / `:1167`. | **Partly.** A generation bump does write one durable `execution_target_revocations` row. Every *denial* the fence then produces returns as a protocol error with no row, no metric and no log line. **★ NARROWED 2026-09-13 by the DE-04+DE-18 fence-denial-audit unit:** the FENCE arm of the denial gap is closed — a `target_revoked` refusal `guardActiveFence` produces on a worker-reachable governed mutator (current throw `job-control.ts:1966`) now writes ONE attributable `security.denied.fence_guard` `activity_log` row keyed to DE-18 (`recordFenceGuardDenial` maps `target_revoked`→DE-18 via `FENCE_GUARD_CROSSING_BY_REASON`, `server/src/services/fence-denial-audit.ts`), `actorId=workerId`, company+org scoped, `details.targetGeneration` present; proven RED-first by `server/src/__tests__/de-04-18-fence-denial-audit.integration.test.ts`. The NON-fence `target_revoked` arms remain unaudited (the admission authority `job-leasing.ts:489` and the session checks `worker-session-auth.ts:166`/`:199`/`:326` — the last runs on EVERY worker request, so its amplification posture must be weighed) plus the placement-coherence drop `job-placement-transaction.ts:59`. The DE-18 denial-audit gap is thereby NARROWED; the formal re-grade + cohort/count re-derivation is DEFERRED (count-cascade avoidance, cf. the DE-13 row). DE-18 stays `partial` and STILL listed in this cohort. (No count or cohort change is taken here.) |
