...
| `packages/worker-daemon/src/poll/poll-loop.ts` | New optional `PollLoopDeps.leaseCandidates`. The candidate is **written on ACK** in `handleOffer` (after `acknowledged`, before `trackHandoff`) and **pruned** in `trackHandoff`'s settle `finally`. A store fault is logged as `lease_candidate_write_failed` and never fails the lease. That lease is then simply not probed at restart, and the control-plane reaper ends it. |
...
- The whole `@armyofagents/worker-daemon` suite: **160 files, 1081 passed, 1 skipped.** Before a test
  ...
  worker reads the response, so a crash taken at "ACK recorded" could precede the write. The test now
  crashes only after every run has reached `create`, which is downstream of the write. It then ran
  green 3 times in a row under full-suite load. That window is real, and it fails safe: a crash
  between the ACK and the write leaves no candidate, so there is no probe and no renewal, and the
  reaper ends the attempt.
...
| 7 | positive control: removing write-on-ACK reds case 1 | Mutation **M1** below (4 cases red). In-suite control ★ 7: lifetime A without a store, so B probes nothing and says `empty`. Also ★ 7: no path configured logs `lease_candidate_store_not_configured` |
...
| # | Mutation | Red |
|---|---|---|
| M1 | **remove the write-on-ACK** (positive control 7) | **4**: ★ 1+5, ★ 2, ★ 6 (row), ★ 8 |
...
_Filled in once the PR's checks complete on the final head._
