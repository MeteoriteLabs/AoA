...
- **🔨 SESSION FRONTIER — TIER 0 + residuals.** The current build front is **TIER 0** (the forward
  ...
  **Still unbuilt:** the real **control-plane KEYPAIR** (the mint call-site is built + inert; only the real key
  is deploy-owed), **DEP-011 Slice 2** (the worker networked composition root + per-run provider resolver +
  credential crossing) + **Slice 3** (the `(compose)` no-op/trust `CleanupAuthority` variant + the reconcile
  idempotent-inversion guard B2 recorded), then Slices 4–5 (credential crossing; deploy = the AM Docker image
  + the compose control-plane-key env). WRK-015's Part 2 (a CI-exercised d1 first-enrol proof) **SPLIT to
...
1. **★ TIER 0 (session/code — the current build front):** **WRK-014 container identity (the hard gate) —
   ...
   owned-labels-capability MINT, inert; a `provider-capability` leaf extracted; independently verified); the real
   control-plane KEYPAIR + DEP-011 Slice 2/3 (worker root + credential crossing; cleanup/reconcile) + Slices 4–5
   remain.** The provider-topology contract is
...
flowchart TD
  ...
  D11["★ TIER 0 — WRK-014 ✅ · WRK-015 ✅ · DEP-012 Unit A ✅ + B1 ✅ + B2 ✅ (the FULL gated wire) + Slice 3 β1 ✅ (ledger + create-gate + TOCTOU lock) + β2 ✅ (real-provider FAIL-CLOSED bin + boundary guard) + DEP-011 Slice 1 ✅ (server-side mint, inert; leaf extracted) · real CP keypair + DEP-011 Slice 2/3 + Slices 4–5 (remaining)<br/>SESS/CODE · gates the fleet (mechanism links 3.3–3.6 already owned)"]:::sess
...
+ DEP-012 Unit A ✅ + B1 ✅ + B2 ✅ + Slice 3 · β1 ✅ + β2 ✅ + DEP-011 Slice 1 ✅ (mint) → the remaining
real control-plane KEYPAIR + DEP-011 Slice 2/3 + DEP-012 Slices 4–5), then the operator deploy (C0), then the run. Everything downstream (**C4–C8**: bridges, sinks, drain, close) is dammed behind
**E7-1 wired**.
...
E7-1 cloud campaign is **DEP-011 Slice 1 ✅ (the server-side mint, inert) → the real control-plane KEYPAIR +
DEP-011 Slice 2/3 (worker root + credential crossing; cleanup/reconcile) + DEP-012 Slices 4–5 (credential /
