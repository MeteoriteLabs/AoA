...
- **🔨 SESSION FRONTIER — TIER 0 + residuals.** The current build front is **TIER 0** (the forward
  ...
  **B2** the AM outbound PULL client (positive-allow-list, res.ok-first, AbortSignal.timeout, never-rejects,
  ...
  refuse-loud, strict-parse) + `/metrics`. **★ PENDING MERGE — PR [#329](https://github.com/MeteoriteLabs/AoA/pull/329)
  (open, mergeable), the FIRST PR-based slice** (prior slices committed DIRECT to `docs/replatform-program`).
...
1. **★ TIER 0 (session/code — the current build front):** **WRK-014 container identity (the hard gate) —
   ...
   is COMPLETE incl. reaper Slice B/C ✅ (VERIFIED green PR #329, pending merge); only the real control-plane KEYPAIR + Slices 4–5 (deploy) remain.** The provider-topology contract is
...
flowchart TD
  ...
  D11["★ TIER 0 — WRK-014 ✅ · WRK-015 ✅ · DEP-012 Unit A ✅ + B1 ✅ + B2 ✅ (the FULL gated wire) + Slice 3 β1 ✅ (ledger + create-gate + TOCTOU lock) + β2 ✅ (real-provider FAIL-CLOSED bin + boundary guard) + DEP-011 Slice 1 ✅ + 2a ✅ + reaper Slice A ✅ (server reclaim) + Slice 2b ✅ (container root) + reaper B/C ✅ (VERIFIED #329, pending merge) — worker→provider CODE PATH COMPLETE · real CP keypair + Slices 4–5 (DEPLOY remaining)<br/>SESS/CODE · gates the fleet (mechanism links 3.3–3.6 already owned)"]:::sess
...
factory + honest cleanup) + reaper Slice A ✅ + Slice 2b ✅ + reaper B/C ✅ (VERIFIED green #329, pending merge) (worker→provider CODE PATH COMPLETE) → the remaining real CP keypair + DEP-012 Slices 4–5 (deploy)), then the operator deploy (C0), then the run. Everything downstream (**C4–C8**: bridges, sinks, drain, close) is dammed behind
**E7-1 wired**.
...
reaper Slice A ✅ + Slice 2b ✅ + reaper B/C ✅ (VERIFIED green #329, pending merge) (worker→provider CODE PATH COMPLETE) → the real control-plane KEYPAIR + DEP-012 Slices 4–5 (credential /
