...
- **🟡 OWED — built, needs a live-infra run (OPERATOR, not session):** **E7-1** staging-canary campaign
  ...
  structurally unreachable until the rollout dial is armed)** (see §1.9.3); **REL-003** DR
...
3. **E7-1 campaign (operator):** arm the canary Org → dispatch ONE real-E2B coding run →
   ...
   unreachable in every checked-in configuration — arm 1 issues no query until a run carries a
   `distributed_job_id`, which only the armed-canary path writes. This campaign's own first action
   ("arm the canary Org") plus dispatch enabled on a worker IS that precondition; a producer is necessary
   and **not** sufficient, and neither is the arming.
...
★★ **Nothing in Track A, B or D flips `capabilityProven`** — and ★★★ **neither would Unit F.** A green
E7-1 proves the MECHANISM, not capability; the verifier computes and prints both since Unit A. **But
"that needs Unit F (output capture)" is measured FALSE, filed 2026-09-06 as E7-F018 (HIGH,
`unowned`):** both arms are structurally unreachable in every checked-in configuration, so a
`workspace_patch` producer is **NECESSARY and NOT SUFFICIENT**. Arm 1 short-circuits at
`e7-distributed-run-verifier-store.ts:200` (`if (run.distributedJobId)`) and issues **no query at
all** — the sole writer of that column is reachable only from the `distributedRolloutState ===
"canary"` block (`heartbeat.ts:5259-5274` → `:5422` → `:6921`), and nothing checked in arms the
rollout dial. What is owed FIRST is a **deployment artefact, not a code unit**: a compose diff
enabling dispatch on a worker, and a rollout JSON naming an Organization canary. **Read E7-F018
before scheduling any producer work** — that chain has been attempted or proposed four times.
