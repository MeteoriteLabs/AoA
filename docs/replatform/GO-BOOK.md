...
- **🔨 SESSION FRONTIER — TIER 0 + residuals.** The current build front is **TIER 0** (the forward
  ...
  **Still unbuilt:** the reaper's **Slice B** (the real `resolveTruth` AM→control-plane lease-truth PULL channel —
  the fork) + **Slice C** (the trigger loop + the AM metric surface) + the real **control-plane KEYPAIR** + **Slice
  5** (deploy = the AM Docker image + the compose control-plane-key env + mTLS) + re-mint-on-renewal (deferred). WRK-015's Part 2 (a CI-exercised d1 first-enrol proof) **SPLIT to
...
1. **★ TIER 0 (session/code — the current build front):** **WRK-014 container identity (the hard gate) —
   ...
   is COMPLETE; the reaper's Slice B/C + the real control-plane KEYPAIR + Slices 4–5 remain.** The provider-topology contract is
...
flowchart TD
  ...
  D11["★ TIER 0 — WRK-014 ✅ · WRK-015 ✅ · DEP-012 Unit A ✅ + B1 ✅ + B2 ✅ (the FULL gated wire) + Slice 3 β1 ✅ (ledger + create-gate + TOCTOU lock) + β2 ✅ (real-provider FAIL-CLOSED bin + boundary guard) + DEP-011 Slice 1 ✅ + 2a ✅ + reaper Slice A ✅ (server reclaim) + Slice 2b ✅ (container root) — worker→provider CODE PATH COMPLETE · reaper Slice B/C + real CP keypair + Slices 4–5 (remaining)<br/>SESS/CODE · gates the fleet (mechanism links 3.3–3.6 already owned)"]:::sess
...
factory + honest cleanup) + reaper Slice A ✅ + Slice 2b ✅ (worker→provider CODE PATH COMPLETE) → the remaining reaper Slice B/C + real CP keypair + DEP-012 Slices 4–5), then the operator deploy (C0), then the run. Everything downstream (**C4–C8**: bridges, sinks, drain, close) is dammed behind
**E7-1 wired**.
...
reaper Slice A ✅ + Slice 2b ✅ (worker→provider CODE PATH COMPLETE) → the reaper's Slice B/C + the real control-plane KEYPAIR + DEP-012 Slices 4–5 (credential /
