...
- **🔨 SESSION FRONTIER — TIER 0 + residuals.** The current build front is **TIER 0** (the forward
  ...
  **Still unbuilt:** the real **control-plane keypair + mint**, **DEP-011**'s through-the-daemon composition seam
  (incl. the `(compose)` no-op/trust `CleanupAuthority` variant + the reconcile idempotent-inversion guard B2
  recorded), then Slices 4–5 (credential crossing; deploy = the AM Docker image + the compose control-plane-key
  env). WRK-015's Part 2 (a CI-exercised d1 first-enrol proof) **SPLIT to
...
1. **★ TIER 0 (session/code — the current build front):** **WRK-014 container identity (the hard gate) —
   ...
   devDep→dep, conformance inherited — verified 98/98+18/18+15/15); the real control-plane mint + DEP-011 daemon
   seam + Slices 4–5 remain.** The provider-topology contract is
...
flowchart TD
  ...
  D11["★ TIER 0 — WRK-014 ✅ · WRK-015 ✅ · DEP-012 Unit A ✅ + B1 ✅ + B2 ✅ (the FULL gated wire) + Slice 3 β1 ✅ (ledger + create-gate + TOCTOU lock) + β2 ✅ (real-provider FAIL-CLOSED bin + boundary guard) · real mint + Slices 4–5 + DEP-011 wire (remaining)<br/>SESS/CODE · gates the fleet (mechanism links 3.3–3.6 already owned)"]:::sess
...
+ DEP-012 Unit A ✅ + B1 ✅ + B2 ✅ + Slice 3 · β1 ✅ + β2 ✅ → the remaining DEP-012 Slices 4–5 + real
control-plane mint + DEP-011 wire), then the operator deploy (C0), then the run. Everything downstream (**C4–C8**: bridges, sinks, drain, close) is dammed behind
**E7-1 wired**.
...
E7-1 cloud campaign is **DEP-012 Slices 4–5 (credential / deploy = AM image + compose control-plane-key env) +
