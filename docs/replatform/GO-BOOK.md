...
- **🔨 SESSION FRONTIER — TIER 0 + residuals.** The current build front is **TIER 0** (the forward
  ...
  **Still unbuilt:** the **server-side sandbox REAPER** (the NEXT slice — reclaims orphaned sandboxes, net-new; the
  MIG-002 sweeper reaps DB rows only) + **DEP-011 Slice 2b** (the container bin + `AOA_WORKER_PROVIDER_URL` +
  boot-root) + the real **control-plane KEYPAIR** + **Slice 5** (deploy = the AM Docker image + the compose
  control-plane-key env + mTLS) + re-mint-on-renewal (deferred cost optimization). WRK-015's Part 2 (a CI-exercised d1 first-enrol proof) **SPLIT to
...
1. **★ TIER 0 (session/code — the current build front):** **WRK-014 container identity (the hard gate) —
   ...
   the honest-orphan cleanup [Option A]; independently verified); the server-side sandbox REAPER (next) + DEP-011
   Slice 2b (bin) + the real control-plane KEYPAIR + Slices 4–5 remain.** The provider-topology contract is
...
factory + honest cleanup) → the remaining server-side REAPER + DEP-011 Slice 2b + real CP keypair + DEP-012 Slices 4–5), then the operator deploy (C0), then the run. Everything downstream (**C4–C8**: bridges, sinks, drain, close) is dammed behind
**E7-1 wired**.
...
server-side sandbox REAPER (next) + DEP-011 Slice 2b (bin) + the real control-plane KEYPAIR + DEP-012 Slices 4–5 (credential /
