...
- **🔨 SESSION FRONTIER — TIER 0 + residuals.** The current build front is **TIER 0** (the forward
  ...
  **wire plumbing**: `packages/provider-wire` codec+driver + `packages/adapter-manager` server, proven
  driver↔server over the key-less mock, 15 tests / 6-of-6 mutants; worker-daemon untouched —
  [`DEP-012-unit-a-result.md`](./epics/E6-deployment-test-harness/tickets/DEP-012-unit-a-result.md)).
  **Still unbuilt:** DEP-012 **Unit B** (the ownership fork + `execute`'s server-side gate + the six
  gate-required ops + redaction) and **DEP-011**'s through-the-daemon composition seam, then Slices 3–5
  (real E2B, credential crossing, conformance, deploy). WRK-015's Part 2 (a CI-exercised d1 first-enrol proof) **SPLIT to
