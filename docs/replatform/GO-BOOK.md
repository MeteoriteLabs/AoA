...
- **🔨 SESSION FRONTIER — TIER 0 + residuals.** The current build front is **TIER 0** (the forward
  ...
  **Still unbuilt:** **Wave β2** (host the real `E2bSandboxProvider` + the new `adapter-manager-boundary.mjs`
  guard + the keyed HOSTED-provider conformance lane), the real **control-plane keypair + mint** (DEP-011),
  and **DEP-011**'s through-the-daemon composition seam (incl. the `(compose)` no-op/trust `CleanupAuthority`
  variant + the reconcile idempotent-inversion guard B2 recorded), then Slices 4–5 (credential crossing,
  deploy). WRK-015's Part 2 (a CI-exercised d1 first-enrol proof) **SPLIT to
...
- **STILL unowned / unbuilt** — the true TIER-0 remainder as of the reconciliation, **three links**: **3.1
  ...
  **DEP-012 Slice 1 is PLUMBING + the FULL gated wire: Unit A ✅ (create+execute wire) + Unit B1 ✅
  ...
  cross-tenant/replay leak + double-provision closed; leak A→B skeptic-refuted).**
  Remaining on 3.7: **Wave β2** (host the real `E2bSandboxProvider` + the `adapter-manager-boundary.mjs` guard
  + keyed HOSTED-provider conformance) + the real control-plane mint + DEP-011's daemon seam (incl. the
  `(compose)` no-op/trust `CleanupAuthority` variant + the reconcile guard B2 recorded) + Slices 4–5
  (credential crossing, deploy) — still unbuilt.
...
1. **★ TIER 0 (session/code — the current build front):** **WRK-014 container identity (the hard gate) —
   ...
   A + B1 + B2 ✅ + Slice 3 · Wave β1 ✅ SHIPPED (the FULL server-side gated wire — create/execute/teardown/
   inspect/list — PLUS the durable idempotency ledger + create-gating + the TOCTOU lock, on the mock); Wave β2
   (real provider + boundary guard + conformance) + the real control-plane mint + DEP-011 daemon seam +
   Slices 4–5 remain.** The provider-topology contract is
...
flowchart TD
  ...
  D11["★ TIER 0 — WRK-014 ✅ · WRK-015 ✅ · DEP-012 Unit A ✅ + B1 ✅ + B2 ✅ (the FULL gated wire) + Slice 3 β1 ✅ (ledger + create-gate + TOCTOU lock) · β2 real provider + real mint + Slices 4–5 + DEP-011 wire (remaining)<br/>SESS/CODE · gates the fleet (mechanism links 3.3–3.6 already owned)"]:::sess
...
(the durable idempotency ledger + create-gating + the TOCTOU lock)**. The honest remainder before the E7-1 cloud
campaign is **DEP-012 Wave β2 (real provider + boundary guard + conformance) + Slices 4–5 (credential / deploy) +
the real control-plane mint + DEP-011 wire** — THEN the operator staging-fleet deploy +
