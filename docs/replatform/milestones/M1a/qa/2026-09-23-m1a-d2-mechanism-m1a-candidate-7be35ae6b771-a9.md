# QA Result — `M1a-D2-MECHANISM`, M1a candidate, `7be35ae6b771`, attempt 9

**Date (UTC):** `2026-09-23`
**Milestone:** `M1a`
**Record path:** `docs/replatform/milestones/M1a/qa/2026-09-23-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a9.md`
**Scope slug:** `m1a-candidate`
**Revision:** `7be35ae6b7719877e61f54ab552de84de8491e7d`
**Attempt:** `9`
**Supersedes:** `docs/replatform/milestones/M1a/qa/2026-09-23-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a8.md`
...
## 0.0000000 Why attempt 9 exists
...
**Provider and template** — **real E2B**, through `packages/sandbox-e2b-provider`, whose declared SDK
constraint at the candidate is **`e2b: "^2.30.5"`** (`packages/sandbox-e2b-provider/package.json:40`).
Sandbox template **`aoa-base`**, defined by `e2b/e2b.Dockerfile` at the candidate.

★★★ **A GAP IN THIS IDENTITY, STATED RATHER THAN PAPERED OVER.** Three of these values are **not**
pinned to an immutable identifier by the evidence:
...
- **The E2B SDK is a RANGE, not a resolved version.** `^2.30.5` is what the manifest declares; the
  lockfile-resolved version is not captured in the evidence bundle.
...
capture the resolved SDK version and an `aoa-base` template build identifier**; the profile digests
...
### 11.1 What a passing attempt 10 would need
...
- **(a)** add a fault-matrix step to `m1-shipped-boot.yml` that runs the `M1a-D2-MECHANISM` profile —
  ...
  run on the candidate under the F8 envelope and file attempt 10 with `Supersedes` naming this path;
  **or**
