# QA Result — `M1a-D2-MECHANISM`, M1a candidate, `7be35ae6b771`, attempt 15

**Date (UTC):** `2026-09-23`
**Milestone:** `M1a`
**Record path:** `docs/replatform/milestones/M1a/qa/2026-09-23-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a15.md`
**Scope slug:** `m1a-candidate`
**Revision:** `7be35ae6b7719877e61f54ab552de84de8491e7d`
**Attempt:** `15`
**Supersedes:** `docs/replatform/milestones/M1a/qa/2026-09-23-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a14.md`
...
## 0.0000000000000 Why attempt 15 exists
...
### 11.1 What a passing attempt 16 would need — three preconditions, not one step

★ *Rewritten in `a11`. The `a10` recipe let an operator satisfy it and still file a non-conforming pass — see §0.000000000.*

**Precondition 1 — run the 22 cases that CAN run here.** Add a fault-matrix step to `m1-shipped-boot.yml` that drives the `M1a-D2-MECHANISM` profile, covering at minimum the clauses the gate names: cancellation, provider failure, daemon-restart reconciliation, the three cleanup paths, the cross-tenant denial surfaces **that this configuration permits** — lease, read, cancel, events, secrets, staged inputs, outputs, cost rows and the four legacy tables — each with a same-tenant positive control, plus `d2m.tenant.control_refused`, the four `d2m.tenant.legacy.*` predicates, both `d2m.tenant.journey.*` arms and `d2m.credential.production_reader_company_predicate`. Then dispatch **one** keyed run on the candidate under the **F8** envelope.

**Precondition 2 — `d2m.tenant.cross.tool_calls` needs a DISPOSITION, not a runner.** ★★★ It cannot be run on this gate: it requires an **armed** tool surface, and the frozen `M1a` configuration disarms it on every replica (§2: `toolSurfaceEnabled null`, `mustBeOff "off"` on all four readings; §9.7). Running it would **violate the freeze checklist this record certifies**, and the declaration already owns it to `CLI-016`/`M1b`. So it requires one of:

- a **gate-owner amendment** that reallocates the case out of `M1a-D2-MECHANISM` to `M1-D2-CODING`   (where `d2c.tenant.cross.tool_calls` already sits) — recorded as such, with an owner; **or**
- an explicit, reviewed **`pending` disposition** on this gate stating that the case is   unsatisfiable under the `M1a` tools exemption and naming where it IS satisfied.

**Neither is a QA owner's act, and neither may be absorbed into a QA record.** A successor that silently counts this case as satisfied, or that arms the surface to run it, fails differently rather than passing.

**Precondition 3 — close the `EVID-01` provider/template identity gap (§2.1).** The successor run must capture, in its evidence bundle:

- an **immutable `aoa-base` template identifier** — a template build id or image digest, not the   template *name* — so a re-registration cannot change what the record attests; and
- the **E2B service version or an equivalent service-side identifier**, or a recorded statement   from the gate owner that none is obtainable and that `EVID-01` is satisfied by the template build   id alone.

The resolved SDK version is already pinned (`e2b@2.30.5`, §2.1) and needs no further work. **Without precondition 3 a successor would file a `pass` carrying the same identity gap this record names as a reason for its `fail`.**

**Then** file attempt 16 with `Supersedes` naming this path.

**The alternative to all three** is that the decision owner amends the gate's clause list — a gate-owner action needing its own review, and a **narrowing of the bar**, which under this programme's rules must be recorded as such, with an owner, and never absorbed into a QA record.

★ **What needs no keyed spend — ONE case, not two.** `d1.reconcile.worker_startup_lease_probe` can be run **keylessly on the spine lane**, whose `worker-b` already has dispatch enabled, because `DEP-018` allocates **restart and reconciliation (`WRK-013`)** to `M1-D1-SPINE` — subject to the live-lease requirement the spine record's §9.1 sets out, since a restart after a completed attempt reconciles an empty candidate store and would check nothing. ★★★ **`d1.provider.worker_terminal_mapping` may NOT be discharged there.** The same contract allocates **provider failure** to `M1a-D2-MECHANISM`, and a **reference-provider** run cannot discharge a **real-E2B** obligation — the deployed worker's terminal mapping stays this lane's, inside precondition 1. *Corrected in `a15`: §11.1 had offered both cases to the spine.*
