# QA Result — `M1a-D2-MECHANISM`, M1a candidate, `7be35ae6b771`, attempt 2

**Date (UTC):** `2026-09-24`
**Milestone:** `M1a`
**Record path:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a2.md`
**Scope slug:** `m1a-candidate`
**Revision:** `7be35ae6b7719877e61f54ab552de84de8491e7d`
**Attempt:** `2`
**Supersedes:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a1.md`
...
independently satisfy the evidence contract. `a1` is immutable and is not edited; this attempt
supersedes it and adds §3.
...
★ **Added in this attempt** (§0). Every command was run by this QA owner against the candidate
worktree or the GitHub API. **No workflow was dispatched.**

| Command | Exit code | Duration | Result summary |
|---|---:|---:|---|
| `gh run view 35920425288 --json databaseId,headSha,conclusion,status,displayTitle,workflowName,event,jobs` | `0` | <5 s | `conclusion success`, `event workflow_dispatch`, head `7be35ae6b7719877e61f54ab552de84de8491e7d`; job `shipped-boot` `{"success": 30}` |
| `gh run view 35919374111 --json …` (keyless rehearsal) | `0` | <5 s | same workflow, same head, `shipped-boot` `{"success": 30}` |
| `gh run download 35920425288 -D /c/m1a-evid/keyed` | `0` | ~10 s | **22** files under `m1-shipped-boot-keyed-35920425288` |
| `find /c/m1a-evid/keyed -type f \| wc -l` | `0` | <1 s | `22` |
| read `candidate.json`, `journey.json`, `tenants.json`, `tenant-set-and-flags.json`, `targets.json`, `workers.json` | `0` | — | §1, §2, §4–§6 |
| read `env-probe-a.json`, `env-probe-b.json` | `0` | — | `verdict absent`; `checked` array counted by hand = **18**; `present: []`; `plantedControl.red: true`; `de08MetadataResidual.reachable: true` (§7) |
| read `reconcile-and-preflight.json`, `presign-probe.txt`, `manifest-check-pre-boot.txt`, `manifest-check-post-rollout.txt`, `verifier-a.txt`, `verifier-b.txt`, `compose-ps.txt` | `0` | — | §2, §6 |
| `for t in a b c; do grep -c icnga1mdlhrayh683vwcl logs-m1-worker-$t.txt; done` | `0`/`1` | <1 s | `1 / 0 / 0` — tenant a's sandbox id appears **only** in tenant a's worker log |
| `for t in a b c; do grep -c iw8yiqpx7b0drx4saibqj logs-m1-worker-$t.txt; done` | `0`/`1` | <1 s | `0 / 1 / 0` — likewise for tenant b |
| `node -e "…JSON.parse(tests/d1/fault-matrix.json)…"` listing every `evidence: "pending"` case per profile | `0` | <1 s | **`M1a-D2-MECHANISM`: 23 cases declared, 23 pending, 0 fired** — the basis of this record's `fail` (§5) |
...
| **positive control for the line above:** `grep -c classifyTenantOutcome scripts/lib/m1-shipped-boot.mjs`; `grep -c shipped-boot .github/workflows/m1-shipped-boot.yml` | `0` | <1 s | `1` and `48` — the files exist and the grep is live, so the `0` above is a real zero and not a mistyped path. *A check that evaluates nothing is not a check.* |
| `grep -n "name:" .github/workflows/m1-shipped-boot.yml` | `0` | <1 s | the 30 steps: validate candidate, checkout, bind, setup, static preflight, containerd store, build+SBOM+sign+admit, pnpm, install, keypair prepare + verify, `boot-core`, `seed`, `apply-rollout`, `assert-tenants`, `provision-targets`, `boot-workers`, `await-workers`, `reconcile`, `probe-presign`, `dispatch`, collect, secret-scan, upload, teardown |
| `node -e "…countProductionCallers(…)" ` for 49 symbols (`scripts/check-gate-clause-wiring.mjs`) | `0` | ~20 s | at the candidate: `observeRun` **5**, `jobBudgetCostBridge` **1**, `createStartupReconciler` **1**, `createArtifactExportSequencer` **1**, `createDistributedExecutionDrain` **1**, `createFenceAwareEgressProxy` **0**, `jobOutputBridge` **0**, `jobAuditBridge` **0** |
| `node scripts/check-gate-clause-wiring.mjs` @ candidate | `0` | ~2 s | `OK (28 wired clause(s), 6 declared dormant, 2 provider-capability claim(s) matched to source)`; dormant set includes `E3-15-budget` and `E5-6-denied-egress` |
| the 38 pure-node `pr.yml` guards + `node scripts/check-evidence-immutability.mjs --base origin/docs/replatform-program` | `0` | ~3 min | `failures: 0` |
...
| F-9 | **the two `M1-D1-SPINE` cases routed here** | `d1.reconcile.worker_startup_lease_probe` and `d1.provider.worker_terminal_mapping` were routed to this lane and **not run**. ★ Their routing was itself invalid — the spine lane's own `worker-b` runs with `AOA_WORKER_DISPATCH_ENABLED: "1"` via `docker/d1/m1-spine.override.yml:137`, so they were never structurally unavailable there (see the spine `a2` §0). **Either way they are certified by no campaign** | `harness` |
...
`tests/d1/fault-matrix.json` therefore stands at **23 declared, 23 `pending`, 0 fired**. Each
`pending` row is properly formed (`pendingKind: "keyed"`, a reason, and
`pendingOwner: "planning session (F8), on the DEP-015 lane"`), so `check-campaign-fault-matrix.mjs`
...
- **Satisfies, as evidence rather than as a gate verdict,** exit criterion 5's *"explicit observation
  ...
  made.
- **Companion record:** `./2026-09-24-m1-d1-spine-m1a-candidate-7be35ae6b771-a2.md`, also `fail`.
- **Ticket state at the candidate:** `DEP-015`, `DEP-017` and `WRK-018` all carry
  `Status: gate_review`. This run is the keyed acceptance each was waiting for, so each is now
  dispositionable — **by a distinct reviewer, not by this QA owner and not by the session that
  dispatched the run.** Until then, exit criterion 1 is unmet.

### 11.1 What a passing attempt 3 would need

Either:

- **(a)** add a fault-matrix step to `m1-shipped-boot.yml` that runs the `M1a-D2-MECHANISM` profile —
  at minimum the clauses the gate names: cancellation, provider failure, daemon-restart
  reconciliation, the three cleanup paths, and the cross-tenant denial set with same-tenant positive
  controls — plus `d2m.credential.production_reader_company_predicate`; then dispatch **one** keyed
  run on the candidate under the F8 envelope and file attempt 3 with `Supersedes` naming this path;
  **or**
- **(b)** the decision owner amends the gate's clause list — a gate-owner action needing its own
  review, and a **narrowing of the bar**, which under this programme's rules must be recorded as
  such, with an owner, and never absorbed into a QA record.

This QA owner recommends **(a)**. The lane exists, it works, and the declaration is already written;
what is missing is a step that runs it. ★ Note that the two cases routed here from the spine
(§5 F-9) can also be run **keylessly on the spine lane**, whose worker already has dispatch enabled —
so they need not wait on any E2B spend.
