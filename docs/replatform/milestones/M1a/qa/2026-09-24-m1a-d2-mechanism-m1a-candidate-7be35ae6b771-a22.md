# QA Result — `M1a-D2-MECHANISM`, M1a candidate, `7be35ae6b771`, attempt 22

**Date (UTC):** `2026-09-24`
**Milestone:** `M1a`
**Record path:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a22.md`
**Scope slug:** `m1a-candidate`
**Revision:** `7be35ae6b7719877e61f54ab552de84de8491e7d`
**Attempt:** `22`
**Supersedes:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a21.md`
...
## 0.00000000000000000000 Why attempt 22 exists
...
- ★★ **Move and promote the routed provider case.** `d1.provider.worker_terminal_mapping` is   declared under **`M1-D1-SPINE`**, not this profile, so Step 4 cannot report it as written: an   `M1a-D2-MECHANISM` bundle naming it raises `evidence:undeclared_case`   (`scripts/lib/campaign-fault-matrix.mjs:364`), and an `M1-D1-SPINE` bundle naming it raises   `evidence:pending_case_reported`. `DEP-018` allocates provider failure **here**, so the   declaration must carry it here: re-declare it in the `M1a-D2-MECHANISM` profile (as   `d2m.provider.worker_terminal_mapping`) and promote it — **or** specify a separate valid   `M1-D1-SPINE` campaign for it. Either way, **before** the keyed spend. *Added in `a20`.*
...
★★ **And the routed provider case — whose id depends on the route Step 0 took.** It is validly this gate's obligation (§5 F-9), but *how* it is reported is route-dependent, so this step is conditional:

- **Route (a), re-declared in this profile:** the bundle reports it as   **`d2m.provider.worker_terminal_mapping`** — the **new** id. Reporting the old   `d1.provider.worker_terminal_mapping` here would raise `evidence:undeclared_case`.
- **Route (b), a separate `M1-D1-SPINE` campaign:** the case is **not** part of this keyed run, and   Step 4 does not cover it. That campaign must be named, and its own bundle reports the case under   its `d1.` id with the declaration promoted there.

**Whichever route is taken, the id in the bundle must be the id the declaration carries.** *Made conditional in `a22`; `a20`/`a21` hard-coded the old id into a step that follows a branch.*
