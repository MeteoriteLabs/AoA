# QA Result — `M1a-D2-MECHANISM`, M1a candidate, `7be35ae6b771`, attempt 24

**Date (UTC):** `2026-09-24`
**Milestone:** `M1a`
**Record path:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a24.md`
**Scope slug:** `m1a-candidate`
**Revision:** `7be35ae6b7719877e61f54ab552de84de8491e7d`
**Attempt:** `24`
**Supersedes:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a23.md`
...
## 0.0000000000000000000000 Why attempt 24 exists

One P2 from the Codex review of PR #591 on `fb7382bc4e`, confirmed and accepted. **No verdict or measurement changes: `Result: fail`, class `harness`.**

★ **§11's gate-effect summary had not absorbed either of `a23`'s Step corrections.** It still said a *"gate-owner disposition"* for `d2m.tenant.cross.tool_calls` — Step 2 proves a disposition that leaves the row `pending` keeps `summary.complete` **false** — and it still counted only the **22** existing runnable rows, omitting the **`d2m.provider.worker_terminal_mapping`** that Step 0 now adds to this profile. An operator reading only the gate effect could leave the tool row pending and omit the routed provider case, and hold incomplete evidence.

Corrected to: an **amendment that REMOVES** the tool row, and **23** cases — the 22 existing runnable rows **plus** the promoted provider-mapping case.

*Sixth site of the summary-propagation class. The gate-effect paragraph has now been wrong after almost every substantive correction, for one structural reason: it restates the remediation in one sentence, so every change to the remediation invalidates it. **A summary of a thing that changes must be re-derived from that thing, not maintained alongside it.***

Every other section is carried forward from `a23` unaltered.

## 0.000000000000000000000 Why attempt 23 existed (carried forward)

Three P2 findings from the Codex review of PR #591 on `fee12f0ffd`, all confirmed at source. **No verdict or measurement changes: `Result: fail`, class `harness`.** Two of them kill routes **I** invented.

★★★ **(1) Route (b) is DELETED. It contradicted the contract, and I wrote it.** `a22` let the terminal-mapping obligation be discharged by a separate **`M1-D1-SPINE`** campaign — while the same record states, correctly, that a **reference-provider** run cannot discharge a **real-E2B** obligation, and `DEP-018` allocates provider failure to **this** gate (`implementation-plan.md` ≈:1316–1319). Taking Route (b) would leave this gate without its required provider-failure evidence after the keyed run. **Only the mechanism-profile route remains**: re-declare the case here as `d2m.provider.worker_terminal_mapping`, promote it, and fire it on the keyed run.

★★ **(2) The "reviewed `pending` disposition" for `d2m.tenant.cross.tool_calls` cannot yield complete evidence either.** `summary.complete = out.length === 0 && summary.pending === 0` (`scripts/lib/campaign-fault-matrix.mjs` ≈:405), and `DEP-018` says so in words: *"`summary.complete` stays `false` while any [pending] remains"* (`implementation-plan.md` ≈:1329–1338). So an operator could obtain the disposition, run all 22 cases, and still hold an **incomplete** bundle. **Only the gate-owner amendment that REMOVES the case from this profile** — reallocating it to `M1-D2-CODING` — can produce completion. The softer option is struck.

★★ **The new-candidate record is a HIGHER ATTEMPT, not attempt 1.** `EVID-02` (`test-gates.md:17`) is explicit: *"A correction, rerun, changed decision, **or later revision** creates a higher attempt with a `Supersedes` link"*, and the QA template repeats it (*"or changed revision"*). I had written "attempt 1 for the new candidate" on the reasoning that a different candidate is a different attestation — **that reasoning is mine and the gate says otherwise**, and filing attempt 1 would sever the eventual passing record from this correction history and be nonconforming in its own right.

§11.1 Step 5 now says **attempt 24 on this chain**, superseding this path.

*Two of my three offered routes were unexecutable — one against the contract, one against the checker. **An alternative is not an alternative until it has been run against both the rule and the machinery.** That is the whole lesson of this review chain in one line.*

Every other section is carried forward from `a22` unaltered.

## 0.00000000000000000000 Why attempt 22 existed (carried forward)

One P2 from the Codex review of PR #591 on `9034f9f63c`, confirmed and accepted. **No verdict or measurement changes: `Result: fail`, class `harness`.**

★ **`a20` gave Step 0 a CHOICE of two declaration routes for the routed provider case, and Step 4 then hard-coded the id from neither of them.** Step 4 named `d1.provider.worker_terminal_mapping` in the single `M1a-D2-MECHANISM` bundle. On **route (a)** (re-declare it here as `d2m.provider.worker_terminal_mapping`) that old id raises **`evidence:undeclared_case`**; on **route (b)** (a separate `M1-D1-SPINE` campaign) the case is not part of the one keyed mechanism run at all. Step 4 is now **conditional on the route Step 0 chose**, and names the id each route produces.

*A recipe that offers a branch must carry that branch through every later step. Leaving one step route-blind is how a plan reads correct and executes wrong — the same class as §0.00000000000000000, and the last of it that this review found.*

Every other section is carried forward from `a21` unaltered.

## 0.0000000000000000000 Why attempt 21 existed (carried forward)

One P2 from the Codex review of PR #591 on `ee170c9a7d`, confirmed and accepted. **No verdict or measurement changes: `Result: fail`, class `harness`.**

★★ **A successor could satisfy every step of §11.1 and still fail this gate on the two findings §5 records as INDEPENDENT of the fault matrix.** §5 lists **F-5** (the operator-visible audit signal is empty: `signals.audit.jobSubmitted` and `securityDenials` both `[]`) and **F-6** (`costEventsForRun: 0`, `costUsd: null` — no priced row on real E2B) as failures in their own right, because the gate text requires *"the operator-visible audit, cost and failure-classification signals journey item 7 names"*. My §11.1 asked only for fault-matrix cases plus the cross-tenant `activity_log` / `cost_events` isolation checks — and **those are not the same evidence**: the isolation cases pass on **planted** rows (the spine's own bundle shows planted ids in `legacyTables`), which proves a tenant filter, not that **the journey emitted** an audit signal or produced a **priced** row.

**Added as Step 4b**, with the assertion stated positively so it cannot be satisfied by a filter test: the journey's own attempt must yield a non-empty audit signal **and** at least one `cost_events` row with **cost > 0** attributable to that attempt, per enabled tenant.

*The shape, once more: I built a recipe out of the failures that were easiest to enumerate — the declared cases — and omitted the two that were written in prose beside them. **Every item in the failure list has to appear in the remediation, or the remediation is a subset pretending to be a plan.***

Every other section is carried forward from `a20` unaltered.

## 0.000000000000000000 Why attempt 20 existed (carried forward)

One P2 from the Codex review of PR #591 on `1d763d39ca`, confirmed at source. **No verdict or measurement changes: `Result: fail`, class `harness`.**

★★ **Step 4 asked for evidence on `d1.provider.worker_terminal_mapping`, but Step 0 promoted only the 22 `M1a-D2-MECHANISM` rows — and that case is declared under the OTHER profile.** Measured in `tests/d1/fault-matrix.json`: `d1.provider.worker_terminal_mapping` (and `d1.reconcile.worker_startup_lease_probe`) are cases of **`M1-D1-SPINE`**, both `pending`. Since `evaluateFaultMatrixEvidence` judges a bundle against **its own named profile**, the recipe was unexecutable in both directions: reporting the case in an `M1a-D2-MECHANISM` bundle raises **`evidence:undeclared_case`** (`scripts/lib/campaign-fault-matrix.mjs:364`, *"the bundle reports case … which profile … does not declare"*), and reporting it in an `M1-D1-SPINE` bundle raises **`evidence:pending_case_reported`**.

So the obligation is real — `DEP-018` allocates provider failure to this gate — but **the declaration does not yet express it**. Step 0 now requires the case to be **moved into the `M1a-D2-MECHANISM` profile as `d2m.provider.worker_terminal_mapping` and promoted**, or a separate valid `M1-D1-SPINE` campaign to be specified for it, **before** any keyed spend.

*Third finding of one shape in two rounds: the recipe assumed the declaration said what the allocation implies. **An obligation a contract creates is not an obligation the harness can report until the declaration carries it.***

Every other section is carried forward from `a19` unaltered.

## 0.00000000000000000 Why attempt 19 existed — the impossible recipe it replaced (carried forward)
...
- **Satisfies, as evidence rather than as a gate verdict,** exit criterion 5's *"explicit observation
  ...
  made.
- ★ **Companion record:** the current head of the **`M1-D1-SPINE`** `Supersedes` chain in this folder — the highest-attempt `./*-m1-d1-spine-m1a-candidate-7be35ae6b771-a*.md` (`a15` when this was written, and deliberately **not** pinned to it; see §0.0000000000000000), **`Result: fail`** on one REQUIRED assertion — the `WRK-013` restart/reconciliation case `DEP-018` allocates to it (§0.000000). **Both `M1a` gate records are `fail`, so exit criterion 8 is unmet on this candidate.** Neither gate can pass on this candidate (§11.1, and the companion's §9.1): each needs harness this revision does not contain, so the next campaign is a **new freeze**. ★ **On that new candidate this lane needs 23 cases fired** — the 22 existing runnable rows **plus the promoted `d2m.provider.worker_terminal_mapping`** that Step 0 moves into this profile — **plus a gate-owner amendment that REMOVES `d2m.tenant.cross.tool_calls` from the profile** (a disposition leaving it `pending` keeps `summary.complete` false), **plus** the §11.1 Step 4b audit and cost signals and the `EVID-01` provider/template identifiers. *Corrected in `a24`: this said "22 RUNNABLE cases plus a gate-owner disposition", which predates `a23`'s Step 0 and Step 2.* *Corrected in `a12`: this read "its 23 declared cases to run".*
...
- ★★ **Move and promote the routed provider case — into THIS profile, and only this profile.** `d1.provider.worker_terminal_mapping` is declared under **`M1-D1-SPINE`**, so Step 4 cannot report it as it stands: an `M1a-D2-MECHANISM` bundle naming it raises `evidence:undeclared_case` (`scripts/lib/campaign-fault-matrix.mjs:364`) and an `M1-D1-SPINE` bundle naming it raises `evidence:pending_case_reported`. `DEP-018` allocates provider failure **here**, so **re-declare it in the `M1a-D2-MECHANISM` profile as `d2m.provider.worker_terminal_mapping` and promote it**, before the keyed spend. ★★★ *`a20`–`a22` also offered a separate `M1-D1-SPINE` campaign as an alternative. That route is **deleted in `a23`**: the spine runs the reference provider, and a reference-provider run cannot discharge a real-E2B obligation — taking it would leave this gate without its required provider-failure evidence.*
...
**Step 2 — `d2m.tenant.cross.tool_calls` needs a gate-owner AMENDMENT that REMOVES it from this profile.** It requires an **armed** tool surface, and the `M1a` freeze checklist disarms it on every replica (§2), so running it would violate the freeze this record certifies; the declaration already owns it to `CLI-016`/`M1b`. ★★★ **And leaving it `pending` with a reviewed disposition is NOT sufficient**, which `a11`–`a22` wrongly offered: `summary.complete = out.length === 0 && summary.pending === 0` (`scripts/lib/campaign-fault-matrix.mjs` ≈:405), and `DEP-018` states it in words — *"`summary.complete` stays `false` while any [pending] remains"*. An operator could take the disposition, run all 22 cases and still hold an **incomplete** bundle. So the case must be **reallocated out of this profile** to `M1-D2-CODING`, where `d2c.tenant.cross.tool_calls` already sits. **That is a gate-owner action and not a QA owner's.**
...
**Step 4 — dispatch ONE keyed run on the NEW candidate**, under the **F8** envelope, covering the clauses the gate names: cancellation, provider failure, daemon-restart reconciliation, the three cleanup paths, the cross-tenant denial surfaces this configuration permits — lease, read, cancel, events, secrets, staged inputs, outputs, cost rows and the four legacy tables — each with a same-tenant positive control, plus `d2m.tenant.control_refused`, both `d2m.tenant.journey.*` arms, `d2m.credential.production_reader_company_predicate`.

★★ **And the routed provider case, reported under its NEW id.** It is validly this gate's obligation (§5 F-9), and Step 0 re-declares it in this profile, so the bundle must name it **`d2m.provider.worker_terminal_mapping`**. Reporting the old `d1.provider.worker_terminal_mapping` here would raise `evidence:undeclared_case` (`scripts/lib/campaign-fault-matrix.mjs:364`), because that id belongs to the `M1-D1-SPINE` profile. **The id in the bundle must be the id the declaration carries.**

★★★ *`a22` made this step conditional on a two-route choice. `a23` **deletes the second route** — a separate `M1-D1-SPINE` campaign — because the spine runs the reference provider and cannot discharge a real-E2B obligation (§0.000000000000000000000). There is one route, so this step is no longer conditional. Striking a route in Step 0 and leaving it standing here would have been the same propagation failure this chain has now recorded five times.*

**Step 4b — prove the AUDIT and COST signals from the journey itself, not from an isolation case.** ★★ These are §5's **F-5** and **F-6**, and they are independent of the fault matrix: a successor can fire every declared case and still fail here. The cross-tenant `activity_log` / `cost_events` cases pass on **planted** rows, which proves a tenant filter, not production emission. So the run must show, **per enabled tenant, for the journey's own attempt**:

- a **non-empty operator-visible audit signal** — on this candidate `signals.audit.jobSubmitted` and   `signals.audit.securityDenials` were both `[]`, which is F-5; and
- **at least one `cost_events` row with cost > 0** attributable to that attempt — on this candidate   `costEventsForRun` was `0` and `costUsd` `null`, which is F-6, and is why **`E3-F037` is not closed   by this chain**. Note the producer half already works: real `claude_local` usage was parsed   (8/618 and 8/597 tokens with non-zero `cachedInputTokens`), so what is unproven is the **priced   row**, not the measurement.

**Both must be asserted positively, with a red control**, or the gate can be reported green while still emitting nothing an operator could see. *Added in `a21`.*

**Step 5 — file the new candidate’s record as ATTEMPT 24 ON THIS CHAIN**, with `Supersedes` naming this path. ★ *Corrected in `a23`: `a19`–`a22` said "attempt 1 for that revision … a different candidate is a different attestation". `EVID-02` (`test-gates.md:17`) says a **later revision** creates a **higher attempt** with a `Supersedes` link, so attempt 1 would sever the eventual pass from this correction history.* The `**Revision:**` field changes to the new candidate; the attempt number keeps climbing.
