# QA Result — `M1-D1-SPINE`, M1a candidate, `7be35ae6b771`, attempt 2

**Date (UTC):** `2026-09-24`
**Milestone:** `M1a`
**Record path:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1-d1-spine-m1a-candidate-7be35ae6b771-a2.md`
**Scope slug:** `m1a-candidate`
**Revision:** `7be35ae6b7719877e61f54ab552de84de8491e7d`
**Attempt:** `2`
**Supersedes:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1-d1-spine-m1a-candidate-7be35ae6b771-a1.md`
**Lane:** `M1-D1-SPINE`
**Result:** `fail`
...
## 0. Why this attempt exists, and what changed from `a1`

`a1` recorded `Result: pass`. **It was wrong, in two independent ways**, both raised by the Codex
...
|---|---:|---:|---|
| `gh run view 35912752449 --json databaseId,headSha,conclusion,status,displayTitle,workflowName,event,jobs` | `0` | <5 s | 3 jobs, all `success`; head `8c01f4e94f8e25d7abaf524e8b266b809c67b8cb`; per-step conclusions as §2 |
| `gh run download 35912752449 -D /c/m1a-evid/spine` | `0` | ~20 s | **41** files across `d1-image-chain-35912752449`, `m1-spine-evidence-35912752449`, `m1-fault-matrix-evidence-35912752449` |
| `git diff --stat 8c01f4e94f8e25d7abaf524e8b266b809c67b8cb 7be35ae6b7719877e61f54ab552de84de8491e7d` | `0` | <1 s | 1 file changed, 120 insertions(+), 0 deletions(−) |
| `git rev-parse <rev>:<dir>` for `server packages ui scripts .github docker tests e2b cli docs`, at both revisions | `0` | <1 s | 9 identical, `docs` differs (§1) |
| `git diff --name-only 8c01f4e94 7be35ae6b \| grep -v /` | `1` (no match) | <1 s | no root-level file changed |
| `node scripts/check-campaign-fault-matrix.mjs` @ candidate | `0` | ~1 s | `OK: tests/d1/fault-matrix.json declares 3 gate profile(s) …`; `M1-D1-SPINE` 28 cases, 3 `pending`, each with kind + reason + owner |
...
| `grep -n AOA_WORKER_DISPATCH_ENABLED docker/d1/*.yml docker-compose.d1.yml` | `0` | <1 s | **one hit: `docker/d1/m1-spine.override.yml:137` = `"1"`** — the §0 correction |
| `grep -n 'SPINE_OVERRIDE_PATH\|dispatch COMPOSED' .github/workflows/d1-merge-train.yml` | `0` | <1 s | override bound at :419 and :678; `dispatch COMPOSED` asserted at :483 |

**Evidence files read in full:** `m1-spine-evidence-…/{passing,post-controls}/manifest.json`,
`…/positive-control/m1-spine-evidence.json`, `…/duplicate-usage-control/m1-spine-evidence.json`,
`m1-fault-matrix-evidence-…/profile/m1-fault-matrix-evidence.json`,
`…/suppressed-control/m1-fault-matrix-evidence.json`, and `tests/d1/fault-matrix.json` at the
candidate.
...
uncertified.** See `./2026-09-24-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a2.md`, which carries
`Result: fail`.
...
### 9.1 What a passing attempt 3 would need
...
3. File attempt 3 with `Supersedes` naming this path.
