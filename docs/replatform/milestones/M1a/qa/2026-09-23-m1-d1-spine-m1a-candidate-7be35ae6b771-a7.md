# QA Result — `M1-D1-SPINE`, M1a candidate, `7be35ae6b771`, attempt 7

**Date (UTC):** `2026-09-23`
**Milestone:** `M1a`
**Record path:** `docs/replatform/milestones/M1a/qa/2026-09-23-m1-d1-spine-m1a-candidate-7be35ae6b771-a7.md`
**Scope slug:** `m1a-candidate`
**Revision:** `7be35ae6b7719877e61f54ab552de84de8491e7d`
**Attempt:** `7`
**Supersedes:** `docs/replatform/milestones/M1a/qa/2026-09-23-m1-d1-spine-m1a-candidate-7be35ae6b771-a6.md`
**Lane:** `M1-D1-SPINE`
**Result:** `fail`
...
## 0.00000 Why attempt 7 exists — the verdict returns to `fail`, and this time on the contract
...
- ★ **`SPINE-MATRIX-3a` — `d1.reconcile.worker_startup_lease_probe`. REQUIRED, and it FAILS.**
  ...
  `dispatch COMPOSED` in its log — so a lease-holding daemon exists and its candidate store is not
  empty. **It did not run**, its `pending` reason is false for this lane, and its named destination
...
| ★ `SPINE-MATRIX-3a` | REQUIRED | `d1.reconcile.worker_startup_lease_probe` — `WRK-013` restart/reconciliation, **allocated to `M1-D1-SPINE` by the `DEP-018` contract** (`epics/E6-deployment-test-harness/implementation-plan.md` ≈:1313–1317; Acceptance item 1: *"Every declared case has a run showing its injection fired"*) | **did not run, and it was runnable.** `docker/d1/m1-spine.override.yml:137` sets `AOA_WORKER_DISPATCH_ENABLED: "1"` on `worker-b`; both certified jobs boot that override (`d1-merge-train.yml` :419, :678); `m1-spine` :483 asserts `dispatch COMPOSED`. Its `pending` reason (*"the D1 workers do not dispatch"*) is **false for this lane**, and its named destination lane fired none of its 23 cases. `d1.restart.control_plane_process` restarts the **control plane** and is not a substitute. ★ *`a6` reclassified this to `OBSERVED`; overruled in `a7` — see §0.00000* | **`fail`** |
...
uncertified.** See `./2026-09-23-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a8.md`, which carries
`Result: fail`.
...
### 9.1 What a passing attempt 8 needs — one case, and no keyed spend

1. ★★★ **Run `d1.reconcile.worker_startup_lease_probe` on this lane.** It is the only thing between
   this campaign and a `pass`. It needs **no E2B spend and no keyed dispatch**: `worker-b` already
   boots with `AOA_WORKER_DISPATCH_ENABLED: "1"` under `docker/d1/m1-spine.override.yml`, already
   composes dispatch (asserted at `d1-merge-train.yml` :483) and already holds leases as the
   `DEP-019` executor, so restarting it reconciles a **non-empty** candidate store. **Alternatively**,
   a reviewed gate amendment may reallocate the case — but that is a gate-owner action, it would be a
   **narrowing of the bar**, and under this programme's rules it must be recorded as such with an
   owner, never absorbed into a QA record.
2. **Repair the two stale `pending` reasons** in `tests/d1/fault-matrix.json`. Both cite *"the D1
   workers do not dispatch"*, which is false of the lane that boots the spine override. A case
   excused on a false premise is the failure class this programme names first: **a check that
   evaluates nothing is not a check, and a false claim of exemption is worse than a missing one.**
3. ★★★ **Carry §4's `SPINE-ROLLBACK-1`, `SPINE-CRIT5-1` and `SPINE-CRIT5-2` forward.** All three are
   required evidence for `M1a` on this lane — criterion 6's rehearsal and criterion 5's spine half
   (plan §6, `E6-D002`) — and a successor that dropped them would pass while observing less than this
   attempt did. **The rollback rehearsal is DONE and must not be re-listed as remediation.**
4. File attempt 8 with `Supersedes` naming this path.
