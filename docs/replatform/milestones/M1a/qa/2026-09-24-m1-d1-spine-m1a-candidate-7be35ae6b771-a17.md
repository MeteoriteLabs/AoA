# QA Result — `M1-D1-SPINE`, M1a candidate, `7be35ae6b771`, attempt 17

**Date (UTC):** `2026-09-24`
**Milestone:** `M1a`
**Record path:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1-d1-spine-m1a-candidate-7be35ae6b771-a17.md`
**Scope slug:** `m1a-candidate`
**Revision:** `7be35ae6b7719877e61f54ab552de84de8491e7d`
**Attempt:** `17`
**Supersedes:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1-d1-spine-m1a-candidate-7be35ae6b771-a16.md`
**Lane:** `M1-D1-SPINE`
**Result:** `fail`
...
## 0.000000000000000000 Why attempt 17 exists

One P2 from the Codex review of PR #591 on `fee12f0ffd`, confirmed at source. **No verdict or measurement changes: `Result: fail`, class `harness`, on `SPINE-MATRIX-3a`.**
...
§9.1 Step 5 now says the new-candidate record is **attempt 18 on this chain**, superseding this path, and the companion continues after its attempt 22. *The correction history and the eventual pass stay in one chain, which is the point of `Supersedes`.*

Every other section is carried forward from `a16` unaltered.

## 0.00000000000000000 Why attempt 16 existed — the new-candidate conclusion (carried forward)

One **P1** from the Codex review of PR #591 on `1d763d39ca`, confirmed at source. **No verdict or measurement changes: `Result: fail`, class `harness`, on `SPINE-MATRIX-3a`.**

★★★ **§9.1 directed a successor to "file attempt 16" — i.e. another attempt on THIS candidate. It cannot be one.** The companion's `a19` established that a post-freeze harness change creates a new candidate; the same reasoning applies here, and I did not carry it across. Measured now: **`d1.reconcile.worker_startup_lease_probe` appears in NO driver.** A repo-wide search finds the case id only in `tests/d1/fault-matrix.json` (its declaration) and in documentation — not in `tests/d1/m1-spine.test.mjs`, not in the fault-matrix driver, not in any workflow. So running it **requires writing the case**, which is a change to `tests/` and the harness, which `qa-handoff-recovery.md` §1 says *"creates a new candidate and new attempt"* — and §1's byte-identity carry-forward does not cover it, because it is not a documentation-only disposition.

**So: `M1-D1-SPINE` cannot be passed on `7be35ae6b7719877e61f54ab552de84de8491e7d`.** Its one blocking case does not exist as executable code at this revision. §9.1 is rewritten to require a **newly frozen candidate and an attempt-1 record for it**, not a higher attempt here.

★ **Both `M1a` gates are now in the same position, and that is the material conclusion of this whole review:** neither `M1-D1-SPINE` nor `M1a-D2-MECHANISM` can pass on this candidate, because each needs harness the candidate does not contain. **`M1a` does not pass on `7be35ae6b…`**, and the next campaign is a new freeze, not another attempt on this one. The evidence this chain records — the rollback rehearsal, criterion 5, the tenant matrix, the cost row, the real-E2B journey — remains valid for what it measured, and is exactly what a successor should carry into that new candidate's records.

*The lesson, twice over: a remediation must be read against the machinery that would execute it. I wrote "run the case" for a case that has no implementation.*

Every other section is carried forward from `a15` unaltered.

## 0.0000000000000000 Why attempt 15 existed — the companion-link loop it closed (carried forward)

One P2 from the Codex review of PR #591 on `22b003a881`, accepted. **No verdict or measurement changes: `Result: fail`, class `harness`, on `SPINE-MATRIX-3a`.**

§8's live companion pointer named mechanism `a16`, which `a17` supersedes.
...
## 0.000000000000 Why attempt 14 existed — the sweep that was actually performed (carried forward)

Two P2 findings from the Codex review of PR #591 on `0b9a85a090`, both accepted. **No verdict or measurement changes: `Result: fail`, class `harness`, on `SPINE-MATRIX-3a`.**

★★★ **`a13` said it had *"swept both remaining claims rather than fixing the flagged line"*. It had not.** Two live sites survived, and both are the classes `a13` claimed to have closed:

1. **§0.1 still carried the non-empty-store inference** — *"a lease-holding daemon does exist on    this lane, its lease-candidate store is **not** empty … They could have run"* — which §9.1    disproves from `poll-loop.ts` (≈:552–558, the candidate is pruned when the handoff settles). It sits    in the paragraph preserving `a2`'s reasoning, so its **wording is kept**, but it is now marked    inline as superseded rather than left to read as a live claim.
2. **§5 item 1 still said *"Two cases unrun without a valid reason"*** — the allocation error `a13`    §8 corrects. Rewritten to **one**.

★ **The method changed, not just the text.** Previous passes fixed the line Codex flagged and asserted a sweep. This pass **grepped both records for each disproved premise** — `not empty|non-empty|could have run|runnable` and `without a valid reason|no valid reason|two of the three` — enumerated every hit, and classified each as *live claim* (fixed), *preserved history* (marked, not rewritten) or *already correct*. **Claiming a sweep is not performing one**, and this chain has now spent five attempts on one propagation class because I kept fixing instances instead of enumerating the class.

Every other section is carried forward from `a13` unaltered.

## 0.00000000000 Why attempt 13 existed (carried forward)

Three P2 findings from the Codex review of PR #591 on `1a9a697438`, all accepted. **No verdict or measurement changes: `Result: fail`, class `harness`, on `SPINE-MATRIX-3a`.**
...
★★★ **(2) §8 said TWO of the three routed cases had no valid reason to be routed away. Only ONE did.** The mechanism companion's `a15` established, from the `DEP-018` allocation, that **provider failure belongs to `M1a-D2-MECHANISM`** — so routing `d1.provider.worker_terminal_mapping` away from the spine is **contract-valid**, even though the *reason* the declaration gives for it is false. `d1.credential.production_reader_company_predicate` is likewise validly routed, by `E6-D003`. So exactly **one** case — `d1.reconcile.worker_startup_lease_probe` — lacked a valid reason to leave this gate, which is why it is the single REQUIRED failure. §8 carried the very allocation error `a15` was minted to fix. Corrected — and note this is the **fourth** site of this propagation class in this chain.

★ **(3) §8's cross-link pointed at mechanism `a14`**, which `a15` supersedes precisely because `a14` offered the real-E2B terminal-mapping obligation to the reference-provider spine lane. Re-pointed at **`a15`**.

**This attempt swept both remaining claims rather than fixing the flagged line**: every statement about which routings are valid, and every intra-record link, was re-read.
...
★ **§8's cross-record link still pointed at mechanism `a12`, which `a13` and `a14` supersede specifically to correct a false pending-owner attribution.** Following it would land a reviewer on evidence known to contain a material factual error. Re-pointed at **`a14`**, which carries the final **21 / 1 / 1** owner split.

*A cross-record link is provenance, so it has to name the attempt a reader should rely on — not the one that happened to be current when the sentence was written.*
...
★ **§9 said the spine lane *"is two clauses short"*. It is ONE clause short.** §5 identifies exactly one REQUIRED failure (`SPINE-MATRIX-3a`) and §9.1 names one blocking case; `SPINE-MATRIX-3b` is **`OBSERVED`**, and the `DEP-018` contract allocates its subject — provider failure / terminal mapping — to `M1a-D2-MECHANISM`, not here. Counting it as a second spine clause would have a successor treat a non-blocking declaration defect as a gate condition of this lane. Corrected to **one**.

*This is the same propagation class as §0.00000000, in the one sentence a reader is most likely to quote.*
...
★★★ **TWO CLAUSES OF THE NEXT SENTENCE ARE SUPERSEDED. Its wording is kept as `a2` wrote it (never rewrite history in a quote); read it with these corrections.** *(a)* *"its lease-candidate store is **not** empty"* and *"They could have run"* are **wrong of the completed campaign** — `poll-loop.ts` (≈:552–558) prunes the candidate when the handoff settles, so the certified run left the store **empty**; dispatch makes the case runnable only **after a candidate is deliberately seeded and held live** (§9.1). *(b)* *"`d1.provider.worker_terminal_mapping` … not structurally unavailable here"* is true of its *reason* but **not** of its allocation: `DEP-018` assigns provider failure to `M1a-D2-MECHANISM`, so routing it away is contract-valid (§8). **Only `d1.reconcile.worker_startup_lease_probe` is this gate's.** *Marked in `a14`.*
...
1. ★ **ONE case unrun without a valid reason** — `SPINE-MATRIX-3a`, §0.00000: `d1.reconcile.worker_startup_lease_probe`, which `DEP-018` allocates to **this** gate. `d1.provider.worker_terminal_mapping` is **validly** the mechanism lane's (`SPINE-MATRIX-3b`, `OBSERVED`); only the *reason* its declaration gives is false. *Corrected in `a14`: this read "Two cases unrun without a valid reason".* **The consequence is the same either way and is concrete: `WRK-013`'s deployed-boot restart behaviour and the deployed worker's terminal mapping are certified by no campaign at all**, because the destination lane fired none of its cases. `WRK-013`'s evidence remains
...
★ **Exactly ONE of those three had no valid reason to be routed at all** — `d1.reconcile.worker_startup_lease_probe`, which `DEP-018` allocates to **this** gate (`SPINE-MATRIX-3a`). The other two are **validly** routed: `DEP-018` allocates provider failure to `M1a-D2-MECHANISM`, and `E6-D003` routes the credential-reader case there — their stated *reasons* are false for this lane, which is a declaration defect, but their *destinations* are contract-correct. **All three are uncertified**, because that destination fired nothing. ★ *Corrected in `a13`: this read "two of those three", the same allocation error the companion's `a15` was minted to fix.* See the current head of the **`M1a-D2-MECHANISM`** `Supersedes` chain in this folder — the highest-attempt `./*-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a*.md` (`a18` when this was written, and deliberately **not** pinned to it; see §0.0000000000000000), which carries
`Result: fail`.
...
- **Retains, at full strength, everything in §4 marked `pass`.** A future attempt may carry those
  measurements forward on this exact revision. The spine lane works; it is **ONE clause short** — `SPINE-MATRIX-3a`. ★ *Corrected in `a11`: this read "two clauses short", which would count the `OBSERVED` `SPINE-MATRIX-3b` as a second blocking condition of this gate; the `DEP-018` contract allocates its subject to `M1a-D2-MECHANISM`.*
...
### 9.1 What a passing successor needs — and it CANNOT be an attempt on this candidate

★★★ **`M1-D1-SPINE` cannot be passed on `7be35ae6b7719877e61f54ab552de84de8491e7d`.** Its one blocking case, `d1.reconcile.worker_startup_lease_probe`, **has no implementation at this revision** — the id appears only in `tests/d1/fault-matrix.json` and in documentation, in no driver. Writing it changes `tests/` and the harness, which `qa-handoff-recovery.md` §1 says creates a **new candidate and new attempt**. *Rewritten in `a16`; `a9`–`a15` told an operator to file a higher attempt here (§0.00000000000000000).*

**Step 1 — implement the case, and make it non-vacuous.** It must:

- **(a)** cause `worker-b` to lease an attempt and **hold it non-terminal** — the candidate row   is written just before the ACK (`handleOffer` → `recordCandidate`), and   `packages/worker-daemon/src/poll/poll-loop.ts` (≈:552–558) **prunes it when the handoff   settles**, so the window is between ACK and settle;
- **(b)** **assert the candidate row is durable on disk** before the restart, so the store is   known non-empty rather than assumed;
- **(c)** restart `worker-b` **while that lease is live**, and assert the composed reconciler   probed **exactly that lease** — one `lease_renew` carrying the prior lifetime's identity —   completing **before** the poll loop starts;
- **(d)** apply founder ruling **F5**: the probe's renewal is its **last**; the lease is fenced,   not re-attached;
- **(e)** carry a **positive control** that reds it — the empty-store path: with no live lease   the reconciler reports its named empty-store reason and the case must **not** count as   satisfied. **Without (e) the case is the vacuous one the matrix exists to prevent.**

**Step 2 — promote its declaration** in the same reviewed change: `check-campaign-fault-matrix.mjs` refuses *"a `pending` case that reported evidence"*, so a run against the unpromoted row reds with `evidence:pending_case_reported`. Repair the two stale `pending` reasons at the same time — both cite *"the D1 workers do not dispatch"*, false of the lane that boots `docker/d1/m1-spine.override.yml`. **A false claim of exemption is worse than a missing check.**

**Step 3 — freeze a NEW candidate** per `qa-handoff-recovery.md` §1, and run this gate on it. ★ The companion gate needs the same new candidate (its own §11.1), so **one freeze serves both** and the two campaigns should be planned together.

**Step 4 — carry this chain's evidence forward.** `SPINE-ROLLBACK-1` (criterion 6's rehearsal, measured clean), `SPINE-CRIT5-1` and `SPINE-CRIT5-2` (criterion 5's spine half, plan §6 / `E6-D002`) and §2.1's `EVID-01` identity must all be re-established on the new candidate. **The rollback rehearsal is DONE here and must not be listed as remediation** — it must be **re-run** there, because a gate record attests one exact revision.

**Step 5 — file the new candidate's record as ATTEMPT 18 ON THIS CHAIN**, with `Supersedes` naming this path. ★ *Corrected in `a17`: `a16` said "attempt 1 for the new candidate … a different candidate is a different attestation, not a higher attempt". That was my reasoning, and `EVID-02` (`test-gates.md:17`) says otherwise — "a correction, rerun, changed decision, **or later revision** creates a higher attempt with a `Supersedes` link". Filing attempt 1 would sever the eventual passing record from this correction history and be nonconforming in its own right.* The record’s `**Revision:**` field is what changes to the new candidate; the attempt number keeps climbing.

**The alternative** is a reviewed gate amendment reallocating the case — a gate-owner action, a **narrowing of the bar**, recorded as such with an owner and never absorbed into a QA record.
