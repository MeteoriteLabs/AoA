# QA Result — `M1a-D2-MECHANISM`, M1a candidate, `7be35ae6b771`, attempt 3

**Date (UTC):** `2026-09-23`
**Milestone:** `M1a`
**Record path:** `docs/replatform/milestones/M1a/qa/2026-09-23-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a3.md`
**Scope slug:** `m1a-candidate`
**Revision:** `7be35ae6b7719877e61f54ab552de84de8491e7d`
**Attempt:** `3`
**Supersedes:** `docs/replatform/milestones/M1a/qa/2026-09-24-m1a-d2-mechanism-m1a-candidate-7be35ae6b771-a2.md`
...
| `cat /c/m1a-evid/keyed/m1-shipped-boot-keyed-35920425288/env-probe-a.json` (and `env-probe-b.json`) | `0` | <1 s | `verdict absent`, `present []`, `plantedControl.red true`, `de08MetadataResidual.reachable true` (§7) |
| `node -e "const j=require('/c/m1a-evid/keyed/m1-shipped-boot-keyed-35920425288/env-probe-a.json'); console.log(j.observed.checked.length);"` | `0` | <1 s | `18` — the credential-class count, counted rather than asserted |
...
| `cat .../reconcile-and-preflight.json .../presign-probe.txt .../manifest-check-pre-boot.txt .../manifest-check-post-rollout.txt .../verifier-a.txt .../workers.json .../targets.json .../tenants.json` | `0` | <1 s | §2, §6 |
...
| `grep -c icnga1mdlhrayh683vwcl .../logs-m1-worker-b.txt` | `1` | <1 s | `0` — absent from tenant b's log (exit 1 is `grep -c`'s no-match) |
| `grep -c icnga1mdlhrayh683vwcl .../logs-m1-worker-c.txt` | `1` | <1 s | `0` |
| `grep -c iw8yiqpx7b0drx4saibqj .../logs-m1-worker-b.txt` | `0` | <1 s | `1` — tenant b's sandbox id in tenant b's log |
| `grep -c iw8yiqpx7b0drx4saibqj .../logs-m1-worker-a.txt` | `1` | <1 s | `0` |
| `grep -c iw8yiqpx7b0drx4saibqj .../logs-m1-worker-c.txt` | `1` | <1 s | `0` |
...
- **Satisfies, as evidence rather than as a gate verdict,** exit criterion 5's *"explicit observation
  ...
  made.
- **Companion record:** `./2026-09-23-m1-d1-spine-m1a-candidate-7be35ae6b771-a3.md`, also `fail`.
...
### 11.1 What a passing attempt 4 would need
...
- **(a)** add a fault-matrix step to `m1-shipped-boot.yml` that runs the `M1a-D2-MECHANISM` profile —
  ...
  run on the candidate under the F8 envelope and file attempt 4 with `Supersedes` naming this path;
  **or**
