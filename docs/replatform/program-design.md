...
- Sandbox egress is default deny. Metadata endpoints, RFC1918 destinations, worker-host control ports, and the AoA data plane are denied unless explicitly required.
  ...
  - **Read `E8-F003` §8 (the census, each row labelled measurement / structural / derived) and `E8-F008` BEFORE proposing a fifth enforcement layer.** The corresponding threat-control crossing `DE-08` (severity `Critical`) reads `deliveryStatus: not-delivered`. **What to do about a Critical control that cannot be enforced at any available layer is a founder decision that has not been taken**; none of these records takes it.
