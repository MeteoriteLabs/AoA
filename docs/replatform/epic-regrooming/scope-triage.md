...
★ **And the `cross-platform-weekly.yml@main` block comes off.** *Added 2026-09-21 (M0 follow-up, founder ruling "blocked, with owner").* `scripts/workflow-verdict-manifest.json` declares that stream `blocked` on the program integration checkpoint, owned by the founder, which is how M0 criterion 2 was met. The block is correct only until `main` carries the program branch's workflow. **M5 does not exit until the block is removed** and the DEP-013 consumer (#358) has read a scheduled `@main` run of the verdict-bearing workflow (E6-F023 option 3) that concluded **`success`**, and reports **zero findings on that stream** after the block is gone. A cancelled or failed run does not satisfy this: once the block is removed, it would be an unowned `not_success` finding again. Only a green run of that workflow shows its verdict-bearing jobs passed. *(Corrected 2026-09-21, Codex on PR #530: an earlier revision required only that a run be read.)* Leaving the block in place after the checkpoint would make "owned" permanent, which the ruling never granted.
