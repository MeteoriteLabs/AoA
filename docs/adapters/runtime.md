...
Agents in AoA do not run continuously.  
...
AoA stores session IDs for resumable adapters.
...
AoA pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. AoA surfaces this as a warning in environment tests, not a hard error.
