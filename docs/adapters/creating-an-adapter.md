...
Build a custom adapter to connect AoA to any agent runtime.
...
2. Build environment with `buildAoAEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `aoa run --watch` using `picocolors`.
...
Make AoA skills discoverable to your agent runtime without writing to the agent's working directory:
