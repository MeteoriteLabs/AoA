# ClipHub: Marketplace for AoA Team Configurations

> The "app store" for whole-company AI teams — pre-built AoA configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for AoA-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | AoA company (agents, projects, governance) |
...
A complete AoA company configuration:
...
Single-agent configurations designed to plug into a AoA org:
...
Portable skill files that any AoA agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with AoA's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum AoA version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target AoA company
...
| `POST` | `/api/listings/:id/install` | Install to AoA company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target AoA company (or create new)
  → ClipHub API calls AoA API to:
  ...
  → Redirect to AoA dashboard with new team running
```
...
### 5.3 Creator: Export from AoA → Publish

```
Running AoA company → "Export as Blueprint" (CLI or UI)
  → AoA exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (AoA brand blue/purple)
...
| Install flow | One-click deploy to AoA company |
...
{
  "targetCompanyId": "uuid",         // Existing AoA company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `aoa-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as AoA
- **Backend**: Node.js API (or extend AoA server)
- **Database**: Postgres (can share AoA's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with AoA auth (or OAuth2)

### 9.2 Integration with AoA

ClipHub can be:
- **Option A**: A separate app that calls AoA's API to install blueprints
- **Option B**: A built-in section of the AoA UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing AoA UI and API.
...
- [ ] Install flow: blueprint → AoA company
