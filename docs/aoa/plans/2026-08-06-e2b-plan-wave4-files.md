...
- **S4 — egress allowlist:** the sandbox provider create/resume path takes an optional `egressAllowlist?: string[]` recorded in **lease metadata** (best-effort on managed E2B — §11 documents managed egress is not fully lockable; **that premise is REFUTED as a capability claim, see `E8-F007`** — the installed SDK exposes `SandboxOpts.network`, and what is inert is AoA passing the list as `metadata`; **and 2026-09-07 measured that `SandboxOpts.network` is ALSO inert at the operator's tier — accepted, validated, echoed back and unenforced, `E8-F008`**). **U6 introduces the param + threads it through stage-in; U11 unions connector hosts + npm into it.**
...
3. **Implement.**
   - In `sandbox-provider-runtime.ts`:
     ...
     - **S4:** add `egressAllowlist?: string[]` to `SandboxProviderAcquireInput` (`:5`). In both `acquireLease` implementations record it in the returned `SandboxProviderLease.metadata` (`metadata.egressAllowlist = input.egressAllowlist ?? []`). On the E2B provider, ALSO pass it through the `Sandbox.create` `metadata` options for managed best-effort recording; **do not throw** if managed E2B cannot enforce it (§11 — managed egress is not fully lockable; enforcement is a self-hosted concern. **That parenthetical is REFUTED as a capability claim — `E8-F007`:** the installed `e2b@2.30.5` exposes a real `network` option; what ships here is `metadata`, which nothing enforces. Whether the operator's tier honours `network` was unmeasured when this line was corrected; **it is now MEASURED and the answer is no — `E8-F008`, run 34085130892**). U11 will union connector hosts + npm into the array supplied at acquire.
