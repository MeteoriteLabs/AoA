...
- **Managed-E2B egress is not fully lockable.** On managed E2B, sandboxes get fairly open egress; the "allowlist" (Q2) may be closer to open in practice there. Acceptable given the blast-radius reframe (§9); a real driver for the self-hosted path for egress-strict customers.
  - **CORRECTION, 2026-09-07 (W10A) — this bullet is REFUTED as a CAPABILITY claim; see finding `E8-F007`.** The installed, lockfile-pinned `e2b@2.30.5` exposes `SandboxOpts.network` (`allowOut`/`denyOut`/`rules`/`allowPublicTraffic`) reaching the `POST /sandboxes` body via `buildNetworkBody`, a `PUT /sandboxes/{sandboxID}/network` `updateNetwork` for a running sandbox, and a `getInfo()` read-back of what the server applied. What the bullet describes accurately is AoA's own behaviour: the allowlist is passed as `Sandbox.create` **metadata**, which is inert (measured — `E8-F003`). **This correction does NOT say egress can be locked.** Whether the operator's E2B tier honours a network body was UNMEASURED when this correction was written; nothing validates the body client-side and the API target is per-company configurable, so a read-back is mandatory before anyone relies on it.
  - **★ MEASURED 2026-09-07 (W11) — finding `E8-F008`, workflow run [`34085130892`](https://github.com/MeteoriteLabs/AoA/actions/runs/34085130892). The tier does NOT honour it IN THE SHAPE THAT WAS TESTED — a `denyOut` CIDR list with no `allowOut`.** The API accepts the `denyOut` set, validates it server-side (a sibling arm was refused with `400: invalid denied CIDR ::ffff:0:0/96`), stores it, and returns it from `getInfo()` **exactly as declared** — and the sandbox reached `169.254.169.254` (HTTP 401) identically to an anti-vacuity sandbox that denied a different range. `updateNetwork` behaves the same. All controls held and the probe's abandon condition did not fire. **And the read-back that the line above calls mandatory PASSES on that unpoliced sandbox** — it verifies what was DECLARED, never what is ENFORCED, so do not carry "a read-back is mandatory" forward as a safeguard. `E8-F008` §3.
    **★★ NARROWED 2026-09-09 (W10B-B).** This bullet read "the tier does NOT honour it" — a claim about the whole `network` surface, generalised from one construction. E2B documents the fine-grained control as default-deny (`denyOut: ({allTraffic}) => [allTraffic]`) **plus** an `allowOut` allowlist, and says domains are unsupported in deny lists — so domain filtering *requires* that form and **it is UNMEASURED**. The arm for it is built (`W10B-egress-enforcement-runbook.md` §13). **★★★ DISPATCHED TWICE 2026-09-09 AND STILL UNMEASURED** — runs [`34328502574`](https://github.com/MeteoriteLabs/AoA/actions/runs/34328502574) and [`34328780645`](https://github.com/MeteoriteLabs/AoA/actions/runs/34328780645), three minutes apart, both `UNRUN — arm-was-never-created`: `Sandbox.create` failed to place that body (`500: Failed to place sandbox … please retry`) while the sibling arms placed seconds apart on the same template. **The documented shape reproducibly fails to place at this tier**, cause unknown and E2B's to explain — *not* a refusal (a `500 … please retry` is not a validation rejection; the same run's IPv6 arm was refused `400: invalid denied CIDR`), *not* transient (it reproduced, with successful siblings), and **not a measurement of enforcement in either direction: `UNRUN` is not `INERT`**. `E8-F008` §8; runbook §13.7. **This is not encouragement:** unmeasured is not "probably works", `DE-08` reads `not-delivered`, this spec's §9 still lists no egress invariant, and nothing in the product passes a `network` body.
