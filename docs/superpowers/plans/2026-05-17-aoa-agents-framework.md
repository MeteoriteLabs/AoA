...
Fixes review Finding 2+3. The dispatcher stays company-agnostic over pending entries (drain all `extractionStatus='pending'`, resolve the agent per company) — BUT only for companies whose extraction agent has an enabled outbox trigger (gating via `listEnabledOutboxAgents` per company). It routes through `runExtractionConsumer`; the consumer (A8) is the thin entrypoint to the runner.

**CORRECTION (A6-exec finding — supersedes the earlier "canary stays 4/4" guardrail):** `extraction-sweeper.test.ts` is coupled to the *pre-Decision-#100* path — it mocks `ensurePlatformAgent`→`"pa-1"`, asserts the consumer receives the **`kind='platform'`** agent, ungated, and its `makeDb` has only 2 select slots (no `listEnabledOutboxAgents` slot). Decision #100 / spec §11 **deliberately supersede** this (extraction → `kind='aoa'`; the platform agent is "migrated/retired"; dispatch is outbox-gated). The shim is therefore behavior-**evolving by design**, NOT behavior-preserving — that canary CANNOT and SHOULD NOT stay 4/4 (forcing it green would mean the framework changed nothing). Resolution: **`extraction-sweeper.test.ts` is RETIRED (`git rm`, rationale in the commit).** The **#99 Phase-1 orphan-reclaim invariants are preserved verbatim** in `dispatcher.ts` and **asserted by `aoa-dispatcher.test.ts`** (the new authoritative dispatch contract — coverage relocates, not lost). `extraction-sweeper-wiring.test.ts` continues to guard the `index.ts` call site via the shim.

**Files:** Create `server/src/services/internal-agent/aoa-agents/dispatcher.ts`; Modify `server/src/services/internal-agent/subagents/extraction-sweeper.ts` (shim); Create `server/src/__tests__/aoa-dispatcher.test.ts`; **Remove `server/src/__tests__/extraction-sweeper.test.ts`** (retired — superseded by aoa-dispatcher.test.ts per Decision #100).
...
```
- [ ] **Step 5: Retire the obsolete canary + verify the new contract.** `git rm server/src/__tests__/extraction-sweeper.test.ts` (it asserts the pre-#100 platform-agent ungated path that A6 deliberately supersedes). Then `cd <worktree>/server && npx vitest run src/__tests__/aoa-dispatcher.test.ts src/__tests__/extraction-sweeper-wiring.test.ts src/__tests__/extraction-consumer.test.ts src/__tests__/extraction-atomic-claim.test.ts`. Expected: ALL GREEN. `aoa-dispatcher.test.ts` is now the authoritative dispatch contract and MUST assert the #99 Phase-1 reclaim (run→failed + entry→pending) — confirm it does. `extraction-sweeper-wiring.test.ts` (guards the `index.ts` call site through the shim) MUST stay green. If `aoa-dispatcher` is red: fix the dispatcher (Phase-1 must be verbatim #99). Do NOT resurrect or hack the retired canary.
- [ ] **Step 6: Commit** `git add server/src/services/internal-agent/aoa-agents/dispatcher.ts server/src/services/internal-agent/subagents/extraction-sweeper.ts server/src/__tests__/aoa-dispatcher.test.ts && git rm server/src/__tests__/extraction-sweeper.test.ts && git commit -m "feat(aoa): generalized dispatcher; retire pre-#100 sweeper canary (superseded by aoa-dispatcher.test.ts)"`
...
- [ ] **Step 1: Eager Commander-Team seed at company-create (CORRECTED — the original premise was wrong).** A9-exec verified: there is **no per-company `internal_agent_config` bootstrap** (only `company-portability.ts:2554` on import; `companyService.create()` doesn't create it — the row is otherwise created lazily when a founder configures Commander in Settings). **Also (skeptical-review finding): the A6 dispatcher gates on `listEnabledOutboxAgents` BEFORE calling `ensureExtractionAgent`, but the outbox trigger is only created by `ensureExtractionAgent` → chicken-and-egg: a company with no pre-existing trigger is skipped forever and extraction never runs.** Resolution (user: "eager at company-create / comes with the app", Decision #100): create a new idempotent `ensureInternalAgentConfig(db, companyId)` (insert a default-defaults row, `.onConflictDoNothing()` on the unique `companyId`) and wire **all three** into `companyService.create()` (`server/src/services/companies.ts`, in `createCompanyWithUniquePrefix`, after `seedCompanyRootFolder`, mirroring its non-fatal `.catch()` pattern), in order: `await ensureInternalAgentConfig(db, companyId); await ensureCommanderAgent(db, companyId); await ensureExtractionAgent(db, companyId);`. Seeding `ensureExtractionAgent` here creates the outbox trigger so the dispatcher gate passes (fixes the deadlock); the dispatcher's existing lazy `ensureExtractionAgent` call stays as an idempotent safety-net. **Intended behavior change (per Decision #100):** new companies now get a default Commander config + the kind='aoa' Commander & Extraction agents — the old "Internal agent not configured — Go to Settings" gate no longer fires for new companies; if a test asserts that gate for a freshly-created company, update it to the new intended behavior (document it, A6/A8-style). **Deferred (flag, out of Plan-A scope):** pre-existing companies created before this wiring need a one-off backfill (or are seeded on next company-touch) — note for a follow-up; the A9.2 integration test uses a fresh company so it is unaffected. New helper file: `server/src/services/internal-agent/aoa-agents/ensure-internal-agent-config.ts` (mirror the ensure-commander idempotent-seed style).
