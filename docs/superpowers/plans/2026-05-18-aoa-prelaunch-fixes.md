...
| **MIN-2 / FX7** | ✅ RESOLVED | `52e30691` | Phase-2 UI verification found the AoA detail page had no manual lifecycle control (founder could pause/resume only via API). Added a founder-gated Pause/Resume(+overflow Terminate) control via `AgentDetailCore`'s existing `headerActions` slot, mirroring the org `AgentDetail` pattern + `agentsApi.pause/resume/terminate`. **NO Invoke** (invoking kind='aoa' via heartbeat is the FX3 runtime-boundary — verified absent in code + live UI). UI tests 10/10, CommanderTeamTab 8/8, UI tsc 0. **Live re-verified**: AoA detail now renders Pause; clicking it flipped Commander→paused end-to-end; zero Invoke affordance. UI-only change (server baseline unaffected). |
| **Phase 2 UI verification** | ✅ PASS | (live) | On the fixed code, fresh seeded company: Commander Team tab lists Commander(lead)+Discussion Extraction(member); AoA detail UUID-routed w/ all tabs + adapter-gated instructions + Config showing the fixed `submit_extracted_items`; create-AoA-agent dialog → kind='aoa'/member; **B1 proven end-to-end** (discussion entry pending→failed + sourceInfo error + founder-visible Reprocess, NOT silent loss); **B2/FX3 proven** (@mention → 0 heartbeat_runs, no dual-exec); M2/FX2 API pause/resume 200. One finding (MIN-2) → FX7. |
| **MIN-1 / FX6** | ✅ RESOLVED | `09aecde5` | (Was pre-existing, not a regression, not the D2 vector.) Fixed per user decision: `kind='aoa'`→founder gate + agent-actor rejection added centrally in `assertCanManageInstructionsPath` (covers `/instructions-path` + `/instructions-bundle` + bundle-file routes — single chokepoint) and inline in `PATCH /agents/:id/permissions`. Mirrors the FX2 pattern. `kind='org'` byte-unchanged. Post-FX6 full regression: **352 files / 0 fail / 3375 tests**, tsc 0. All aoa-mutation surfaces now founder-gated. |
