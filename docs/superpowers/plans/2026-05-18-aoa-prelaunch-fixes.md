...
**Fix (mirror the legacy `extraction.ts` failure terminalizer):**
1. Runner: track `claimedEntryId` (set only after the atomic claim at `runner.ts:55-72` succeeds). In the `catch`, if set, atomically transition that entry `processing → failed` **guarded on `extractionStatus='processing' AND extractionRunId = thisRunId`**, mirroring `extraction.ts`'s failure branch exactly: set `extractionStatus='failed'`, record error in `sourceInfo` (same `jsonb_set` shape), publish the `discussion.extraction.failed` LiveEvent, write the founder notification — read `extraction.ts` at exec time, do not invent the shape.
2. Dispatcher Phase-1 safety net: extend the orphan query so `processing` entries whose **linked run is already `failed`** are also reclaimed → terminalized to `failed` + notification (NOT → `pending`; a failed run is a completed attempt — resetting would infinite-loop on a broken adapter). Keep the existing `running`+stale → `pending` branch byte-unchanged. Both guarded on `extractionStatus='processing'`.
...
**Files:** Modify `server/src/services/internal-agent/aoa-agents/runner.ts`, `server/src/services/internal-agent/aoa-agents/dispatcher.ts`, `server/src/services/internal-agent/aoa-agents/ensure-extraction-agent.ts`. Reference (read only): `server/src/services/extraction.ts` (failure branch — authoritative shape), `server/src/services/internal-agent/tools/submit-extracted-items.ts` (`name`). Tests: new `server/src/__tests__/aoa-runner-failure-terminalize.test.ts`, extend `aoa-dispatcher.test.ts`, new/extended `aoa-extraction-instruction-contract.test.ts`.
...
- [ ] **Step 2 (failing tests):** (a) runner: claimed entry + throwing adapter → entry → `failed` (guarded where-clause incl. `extractionRunId=runId`), failed LiveEvent published, founder notification inserted, run `failed`. (b) dispatcher: `processing` entry w/ linked `failed` run → terminalized `failed`+notify; `running`+stale entry still → `pending`. (c) contract: `EXTRACTION_INSTRUCTION` contains `submit_extracted_items` and NOT `submit-extracted-items`; `EXTRACTION_AGENT_TOOL_ALLOWLIST[0] === "submit_extracted_items"`. Run → FAIL.
...
- [ ] **Step 6 (commit):** `git add server/src/services/internal-agent/aoa-agents/{runner,dispatcher,ensure-extraction-agent}.ts server/src/__tests__/<test files> && git commit -m "fix(aoa): terminalize entry on extraction failure + correct seeded tool name (B1; §6.3/§17)"`
