...
- [ ] **Step 3: Run → FAIL.**
- [ ] **Step 4: Implement** — the bridge/`authorize-tool` resolves the calling AoA agent's `runtimeConfig.aoa.toolAllowlist` and intersects it with the existing capability gate: effective = allowlist ∩ enabledCapabilities-derived set; default-deny when allowlist absent. **Finding R4 — both seeds, landed together with the default-deny (no window where a needed tool is denied):**
  - `ensureExtractionAgent` (Plan A) seed → `runtimeConfig.aoa.toolAllowlist: ['submit-extracted-items']`.
  - `ensureCommanderAgent` (Plan A) seed → `runtimeConfig.aoa.toolAllowlist` including `'delegate-to-subagent'` (B3) + Commander's existing tool needs (derive from `internal_agent_config.enabledCapabilities`; the lead is broad but still explicit, not exempt — least privilege applies to the lead too).
  - Idempotent backfill: an upgrade migration/seed-touch sets the allowlist on existing rows so the default-deny does not strand already-seeded agents. The default-deny code + both seed updates ship in **one commit** (ordering safety).
- [ ] **Step 5: Run → PASS.**
