...
| **F2** | ✅ RESOLVED | `dde5bf38` + `e0d2083b` | submit-extracted-items.ts now emits `discussion.extraction.completed` (exact extraction.ts:630-638 shape) after the terminal write. discussionId resolved **unconditionally** (single lookup reused by I-1 increment + the event) so empty-items completions emit too. `aoa-submit-extracted-items.test.ts` empty-items test's incidental `selects.length===0` (pre-F2 strategy) updated to `===1` with documented rationale — real no-increment contract stays enforced by the sibling `pendingUpdates===0`. **NIT tightened (`e0d2083b`, per user decision):** emit gated on the guarded terminal write's `.returning()` being non-empty (mirrors runner.ts M2 atomic-claim / extraction.ts linear-success-branch convention) so a concurrent terminalizer can't cause a double-emit; tool return value unchanged (idempotent no-op when already terminal); test mocks faithfully shape-aligned (no assertion weakened) + new 0-rows negative test. |
