...
This design document specifies the architecture, data model modifications, and service updates required to resolve 6 active GitHub issues in the AoA (Army of Agents) codebase:
...
6. **Issue #114**: Cross-platform E2E CI execution on `windows-latest` via external PostgreSQL container.
...
1. **Schema Update (`packages/db/src/schema/internal_agent.ts`)**:
   ...
   - Migration file: `0196_add_last_event_at_to_internal_agent_runs.sql`.
2. **Heartbeat Leasing (`server/src/services/internal-agent/aoa-agents/runner.ts`)**:
   - Inside stream execution callbacks (`onLog`, `onMeta`, tool execution), maintain an in-memory timestamp. If `now - lastWrite > 30000ms`, update `internal_agent_runs.last_event_at = now()`.
3. **Single-Consumer Consolidation (`runner.ts` & `sweep-controller.ts`)**:
   ...
   - Wrap `sealRunActions` and `internalAgentRuns.status = 'completed'` in a single DB transaction in `runner.ts`.
...
2. **Windows E2E CI (`.github/workflows/cross-platform-weekly.yml`)**:
   - Add PostgreSQL service container to `windows-latest` matrix leg.
   - Supply `DATABASE_URL: postgres://postgres:postgres@localhost:5432/aoa` to Playwright step.

---

## 3. Testing & Verification Plan

1. **Type Checking & Linting**: `pnpm -r typecheck`
2. **Unit & Integration Suite**: `pnpm test:run`
3. **Build Validation**: `pnpm build`
4. **Manual & Targeted Checks**:
   - Verify `extractTextFromBuffer` execution time on Windows host.
   - Validate Drizzle migration `0196` generation and application.
   - Verify `DELETE /artifacts/:id` permission behavior.
