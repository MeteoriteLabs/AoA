...
description: >
  Technical guide for creating a new AoA agent adapter. Use when building
...
# Creating a AoA Agent Adapter

An adapter bridges AoA's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `aoa run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@armyofagents/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@armyofagents/adapter-utils` (types) or `@armyofagents/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell AoA to forget the stored session
}
...
```json
{
  "name": "@armyofagents/adapter-<name>",
  "version": "0.0.1",
  "private": true,
  "type": "module",
  "exports": {
  ...
  "dependencies": {
    "@armyofagents/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including AoA agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@armyofagents/adapter-utils/server-utils`)
2. **Build environment** — call `buildAoAEnv(agent)` then layer in `AOA_RUN_ID`, context vars (`AOA_TASK_ID`, `AOA_WAKE_REASON`, `AOA_WAKE_COMMENT_ID`, `AOA_APPROVAL_ID`, `AOA_APPROVAL_STATUS`, `AOA_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `AOA_API_URL` | Server's own URL |
| `AOA_RUN_ID` | Current run id |
| `AOA_TASK_ID` | `context.taskId` or `context.issueId` |
| `AOA_WAKE_REASON` | `context.wakeReason` |
| `AOA_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `AOA_APPROVAL_ID` | `context.approvalId` |
| `AOA_APPROVAL_STATUS` | `context.approvalStatus` |
| `AOA_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `AOA_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `aoa run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@armyofagents/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@armyofagents/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use AoA API
...
import { parseMyAgentStdoutLine } from "@armyofagents/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@armyofagents/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@armyofagents/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so AoA wipes the stale session
...
Import from `@armyofagents/adapter-utils/server-utils`:
...
| `buildAoAEnv(agent)` | Standard `AOA_*` env vars |
...
- Package name: `@armyofagents/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your AoA work."`
...
### AoA Skills Injection

AoA ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `aoa` API skill and the `aoa-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with AoA internals, break git status, and potentially leak into commits.
...
async function buildSkillsDir(): Promise<string> {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(AOA_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks AoA skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The AoA skills (like `aoa` and `aoa-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the AoA API to report status), use explicit instructions in the prompt template: "Use the aoa skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between AoA's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `AOA_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the AoA API and GitHub should not have access to arbitrary hosts.
